...
final class SpeechBridge: NSObject, WKScriptMessageHandler, AVSpeechSynthesizerDelegate {
    ...
    /// app-shell/speech_shim.js, shared with the Android app (whose bridge
    /// speaks the same protocol through the same port name).
    static var shimSource: String { AppShell.shellFile("speech_shim.js") }

    /// The shim, as documented. Injected at document start. It defines `window.speechSynthesis` and
