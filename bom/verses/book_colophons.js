// Book-of-Mormon plate colophons (interlinear) — loaded before per-book verse files
(function() {
var n2_colophonVerses = [
  { num:"", english:"An account of the death of Lehi. Nephi's brethren rebel against him. The Lord warns Nephi to depart into the wilderness. His journeyings in the wilderness, and so forth.", words:[
    ["דִּבְרֵי","An account of"],
    ["מוֹת","the death of"],
    ["לֶחִי","Lehi"],
    ["וַיִּפְשְׁעוּ","rebel"],
    ["אֲחֵי","the brethren of"],
    ["נֶפִי","Nephi"],
    ["בּוֹ","against him"],
    ["וַיַּזְהֵר","warns"],
    ["יְהוָה","The Lord"],
    ["אֶת־נֶפִי","Nephi"],
    ["לָצֵאת","to depart"],
    ["אֶל־הַמִּדְבָּר","into the wilderness"],
    ["וּמַסָּעָיו","His journeyings"],
    ["בַּמִּדְבָּר","in the wilderness"],
    ["וְהָלְאָה","and so forth"],
    ["׃",""]
  ]}
];
var jc_colophonVerses = [
  { num:"", english:"The words of his preaching unto his brethren. He confoundeth a man who seeketh to overthrow the doctrine of Christ. A few words concerning the history of the people of Nephi.", words:[
    ["דְּבָרָיו","his words"],
    ["אֲשֶׁר","which"],
    ["דִּבֵּר","he spoke"],
    ["אֶל־עַם","unto the people of"],
    ["נֶפִי","Nephi"],
    ["בּוֹ","in him"],
    ["הִשְׁבִּית","he had overcome"],
    ["מַחֲלֹקֶת","a division"],
    ["לְמַעַן","in order that"],
    ["יִוָּדַע","he shall be known"],
    ["מָשִׁיחַ","the Messiah"],
    ["אֲשֶׁר","who"],
    ["הוּא","he"],
    ["הַמֶּלֶךְ","the king"],
    ["קְדוֹשׁ","holy"],
    ["יִשְׂרָאֵל","Israel"],
    ["לָכֵן","therefore"],
    ["הֶעֱלָה","he taught"],
    ["דִּמְיוֹן","the similitude"],
    ["עֵץ","a tree"],
    ["הַזַּיִת","the olive"],
    ["הַתַּרְבּוּתִי","of cultivation"],
    ["וְהַיַּעַר","and the forest"],
    ["וַיַּזְכִּיר","and he reminded"],
    ["אֶת־הַמִּצְוֹת","the commandments"],
    ["אֲשֶׁר","which"],
    ["צִוָּהוּ","commanded him"],
    ["נֶפִי","Nephi"],
    ["אָחִיו","his brother"],
    ["׃",""]
  ]}
];
var al_colophonVerses = [
  { num:"", english:"The account of Alma, who was the son of Alma, the first and chief judge over the people of Nephi, and also the high priest over the Church. An account of the reign of the judges, and the wars and contentions among the people. And also an account of a war between the Nephites and the Lamanites, according to the record of Alma, the first and chief judge.", words:[
    ["דִּבְרֵי","The account of"],
    ["אַלְמָא","Alma"],
    ["אֲשֶׁר","who"],
    ["הָיָה","was"],
    ["לְבֶן־אַלְמָא","son of Alma"],
    ["הַכֹּהֵן","the priest"],
    ["הַגָּדוֹל","the great"],
    ["הָרִאשׁוֹן","the first"],
    ["עַל־עֲדַת","over the church of"],
    ["הָאֵל","God"],
    ["וְדִבְרֵי","and an account of"],
    ["מַלְכוּת","the reign of"],
    ["הַשֹּׁפְטִים","the judges"],
    ["וְלַמִּלְחָמוֹת","and their wars"],
    ["וְלַמַּחֲלֹקֶת","and their dissensions"],
    ["בִּימֵי","in the days of"],
    ["הָעָם","the people"],
    ["עַל־פִּי","according to"],
    ["דִּבְרֵי","The account of"],
    ["אַלְמָא","Alma"],
    ["הַכֹּהֵן","the priest"],
    ["הַגָּדוֹל","the great"],
    ["הָרִאשׁוֹן","the first"],
    ["עַל־עֲדַת","over the church of"],
    ["הָאֵל","God"],
    ["׃",""]
  ]}
];
var he_colophonVerses = [
  { num:"", english:"An account of the Nephites. Their wars and contentions, and their dissensions. And also the prophecies of many holy prophets, before the coming of Christ, according to the records of Helaman, who was the son of Helaman, and also according to the records of his sons, even down to the coming of Christ. And also many of the Lamanites are converted. An account of their conversion. An account of the righteousness of the Lamanites, and the wickedness and abominations of the Nephites, according to the record of Helaman and his sons, even down to the coming of Christ, which is called the book of Helaman, and so forth.", words:[
    ["דִּבְרֵי","An account of"],
    ["בְּנֵי","the sons of"],
    ["הֵילָמָן","Helaman"],
    ["עַל־פִּי","according to"],
    ["דִּבְרֵיהֶם","their record"],
    ["וְעַל־פִּי","and according to"],
    ["דִּבְרֵי","An account of"],
    ["עַמָּם","their people"],
    ["וּבוֹ","and in it"],
    ["נִכְלָל","is contained"],
    ["דִּבְרֵי","An account of"],
    ["מִלְחֲמוֹתָם","their wars"],
    ["וּלְמַחֲלֹקֹתָם","and their contentions"],
    ["וּלְמַחֲלֹקֶת","and dissension of"],
    ["עַמָּם","their people"],
    ["וְדִבְרֵי","and a remembrance"],
    ["נְבוּאוֹתֵיהֶם","their prophecies"],
    ["רַבִּים","many"],
    ["מִנְּבִיאִים","of prophets"],
    ["קְדוֹשִׁים","holy"],
    ["טֶרֶם","before"],
    ["בּוֹא","the coming of"],
    ["הַמָּשִׁיחַ","the Messiah"],
    ["עַל־פִּי","according to"],
    ["הָאִגְּרוֹת","the epistles"],
    ["אֲשֶׁר","which"],
    ["נִכְתְּבוּ","were written"],
    ["בִּידֵי","by the hand of"],
    ["בְּנֵי","the sons of"],
    ["הֵילָמָן","Helaman"],
    ["עַד־בּוֹא","until the coming of"],
    ["הַמָּשִׁיחַ","the Messiah"],
    ["וְכֵן","and thus"],
    ["יִקָּרֵא","shall be called"],
    ["אֶת־מַרְבִּית","the greater part of"],
    ["סֵפֶר","the book of"],
    ["הֵילָמָן","Helaman"],
    ["עַל־פִּי","according to"],
    ["דִּבְרֵי","An account of"],
    ["בְּנֵי","the sons of"],
    ["הֵילָמָן","Helaman"],
    ["׃",""]
  ]}
];
var tn_colophonVerses = [
  { num:"", english:"And Helaman was the son of Helaman, who was the son of Alma, who was the son of Alma, being a descendant of Nephi who was the son of Lehi, who came out of Jerusalem in the first year of the reign of Zedekiah, the king of Judah.", words:[
    ["וְהֵילָמָן","and Helaman"],
    ["הָיָה","was"],
    ["לְבֶן־הֵילָמָן","son of Helaman"],
    ["בֶּן־אַלְמָא","son of Alma"],
    ["בֶּן־אַלְמָא","son of Alma"],
    ["אֲשֶׁר","who"],
    ["יָצָא","came forth"],
    ["מִזֶּרַע","from the seed of"],
    ["נֶפִי","Nephi"],
    ["הוּא","he"],
    ["הָיָה","was"],
    ["לְבֶן־לֶחִי","son of Lehi"],
    ["אֲשֶׁר","who"],
    ["יָצָא","came forth"],
    ["מִיְּרוּשָׁלַיִם","from Jerusalem"],
    ["בִּשְׁנַת","in the year"],
    ["אַחַת","one"],
    ["לְמַלְכוּת","of the reign of"],
    ["צִדְקִיָּהוּ","Zedekiah"],
    ["מֶלֶךְ","king of"],
    ["יְהוּדָה","Judah"],
    ["׃",""]
  ]}
];
var fn_colophonVerses = [
  { num:"", english:"An account of the people of Nephi, according to his record.", words:[
    ["דִּבְרֵי","An account of"],
    ["עַם","the people of"],
    ["נֶפִי","Nephi"],
    ["עַל־פִּי","according to"],
    ["דְּבָרָיו","his record"],
    ["׃",""]
  ]}
];
var et_colophonVerses = [
  { num:"", english:"The record of the Jaredites, taken from the twenty-four plates found by the people of Limhi in the days of King Mosiah.", words:[
    ["דִּבְרֵי","The record of"],
    ["בְּנֵי","the sons of"],
    ["יָרֶד","Jared"],
    ["אֲשֶׁר","who"],
    ["נָסְעוּ","journeyed"],
    ["מִן־הַמִּגְדָּל","from the tower"],
    ["בָּעֵת","at the time"],
    ["אֲשֶׁר־בָּלַל","when confounded"],
    ["יְהוָה","the Lord"],
    ["אֶת־שְׂפַת","the language of"],
    ["הָעָם","the people"],
    ["וְהֵפִיצָם","and scattered them"],
    ["בְּכׇל־פְּנֵי","upon all the face of"],
    ["הָאֲדָמָה","the earth"],
    ["עַל־פִּי","according to"],
    ["דִּבְרֵי","The record of"],
    ["מוֹרוֹנִי","Moroni"],
    ["אֲשֶׁר","who"],
    ["לָקַח","took"],
    ["אֶת־דְּבָרָיו","his record"],
    ["מֵעֶשְׂרִים","from twenty"],
    ["וְאַרְבָּעָה","and four"],
    ["הַלּוּחוֹת","plates"],
    ["אֲשֶׁר","which"],
    ["מָצְאוּ","found"],
    ["עַם","the people of"],
    ["לִמְחִי","Limhi"],
    ["הַנִּקְרָאִים","who are called"],
    ["לוּחוֹת","plates of"],
    ["אֶתֶר","Ether"],
    ["׃",""]
  ]}
];

renderVerseSet(n2_colophonVerses, '2n-colophon-verses');
renderVerseSet(jc_colophonVerses, 'jc-colophon-verses');
renderVerseSet(al_colophonVerses, 'al-colophon-verses');
renderVerseSet(he_colophonVerses, 'he-colophon-verses');
renderVerseSet(tn_colophonVerses, '3n-colophon-verses');
renderVerseSet(fn_colophonVerses, '4n-colophon-verses');
renderVerseSet(et_colophonVerses, 'et-colophon-verses');
})();
