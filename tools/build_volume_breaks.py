...
# ---- and the reader's own ear (2026-09-19) ---------------------------------
# Auditing 1 Nephi 1 by ear found two defects the English commas carry over
# again and again. "Therefore, I was taught" puts a comma after a bare
# connective, and Hebrew never breathes after לָכֵן alone — nor after וְעַתָּה,
# הִנֵּה, עַל־כֵּן, אָכֵן, אַךְ, כִּי, אֲשֶׁר, וַיְהִי or the rest below. And
# "my father, Lehi," / "I, Nephi," puts commas round an apposition that
# Hebrew says in one breath: a break between אָבִי or אֲנִי and the name that
# follows is dropped whenever the next word is not a וְ-clause.
_NO_BREAK_AFTER = set(u'''לכן ולכן ועתה עתה הנה והנה על־כן ועל־כן אכן אך ואך ואף אף כי ואשר אשר גם וגם אז ואז
ויהי והיה כאשר וכאשר אם ואם או פן למען ולמען אולם אבל רק כי־אם עוד ועוד לו ולו הן ובכן'''.split())
_TITLE_BEFORE_NAME = set(u'''אני אנכי אבי אמי אחי אחיו אביו אמו בני בנו בתו אשתו אביהם אחיהם אחיך אביך בנך'''.split())
# points off, the maqqef kept: עַל־כֵּן must still read על־כן to be found
_POINTS_NOT_MAQQEF = re.compile(u'[\u0591-\u05BD\u05BF-\u05C7]')
def _bare(w):
    return _POINTS_NOT_MAQQEF.sub('', w).replace(u'\u05C3', '')
def drop_unnatural(br, speak):
    """Remove a break that follows a bare connective, or that splits a title
       from the name in apposition to it (אָבִי לֶחִי)."""
    out, cut = [], 0
    for i, c in br:
        if 0 <= i < len(speak) - 1:
...
def build(vol):
    ...
    for key, ref, toks in verses(vol, bk, idx):
        ...
        vetoed[0] += cut
        br, cut = drop_unnatural(br, speak)
        unnatural[0] += cut
        if br:
            table[key] = [[i, c] for i, c in br]
        else:
            plain += 1
    # THE HAND HAS THE LAST WORD. tools/phrase_break_overrides.json holds verses
    # audited by ear against the Hebrew clause structure; each replaces the
    # generated entry outright, so a rebuild never undoes an audit.
    hand_path = os.path.join(HERE, 'phrase_break_overrides.json')
    hand = {}
    if os.path.exists(hand_path):
        hand = json.load(io.open(hand_path, encoding='utf-8')).get(vol, {})
        for k, v in hand.items():
            table[k] = [list(x) for x in v]
    ...
    with io.open(path, 'w', encoding='utf-8') as fh:
        fh.write(u'// %s_phrase_breaks.js — auto-generated by tools/build_volume_breaks.py.\n'
                 u'// DO NOT EDIT (hand audits go in tools/phrase_break_overrides.json).\n'
                 u'// Where each verse breathes: [index, weight], the index into\n'
    ...
    print('     %s verse(s) phrased by the Masoretes themselves (MT parallel)'
          % format(fromMT[0], ','))
    if hand:
        print('     %s verse(s) phrased by hand (tools/phrase_break_overrides.json)' % len(hand))
    print('     %s break(s) dropped: after a bare connective, or between a title and its name'
          % format(unnatural[0], ','))
