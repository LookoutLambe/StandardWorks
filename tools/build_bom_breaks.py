...
# ------------------------------------------------------------ extraction
# A QUESTION RISES, AND A FALLING CONTOUR ON ONE IS SIMPLY WRONG. 1,949 verses
# in the six volumes end in a question mark and every one of them was being
# read as a statement. The printed English is the only thing that knows, and it
# knows at build time, so the verse keys ship beside the breaks in the same
# file — no new request, no new precache entry.
...
def main():
    ...
    EN = english(root)
    table, ask, verses, noeng, plain = {}, {}, 0, 0, 0
    for f, en, pre in BOOKS:
        for (ch, v), (ref, toks) in sorted(corpus(root, f, pre).items()):
            ...
            if not ent: noeng += 1
            if ent and ent.rstrip().endswith('?'):
                ask['%s|%d|%d' % (en, ch, v)] = 1
    ...
    ask_body = json.dumps(ask, ensure_ascii=False, separators=(',', ':'))
