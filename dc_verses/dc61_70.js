// dc_verses/dc61_70.js — D&C verse data
(function() {
var dc61_ch1Verses = [
  { num: "א", words: [
    ["הִנֵּה","behold"],
    ["וְהַאֲזִינוּ","and hearken"],
    ["לְקוֹל","unto the voice of"],
    ["אֲשֶׁר","him who"],
    ["לוֹ","unto him"],
    ["כׇּל","has all"],
    ["כֹּחַ","power"],
    ["הַהֹוֶה","who is"],
    ["מֵעוֹלָם","from everlasting"],
    ["וְעַד","and unto"],
    ["עוֹלָם","everlasting"],
    ["אָלֶף","Alpha"],
    ["וְתָו","and Omega"],
    ["הָרֵאשִׁית","the beginning"],
    ["וְהָאַחֲרִית׃","and the end"]
  ]},
  { num: "ב", words: [["הִנֵּה","behold"], ["אָמֵן","verily"], ["כֹּה","thus"], ["אָמַר","says"], ["יְהוָה","the Lord"], ["לָכֶם","unto you"], ["זִקְנֵי","O elders of"], ["כְּנֵסִיָּתִי","my church"], ["הַנֶּאֱסָפִים","who are assembled"], ["בַּמָּקוֹם","upon the place"], ["הַזֶּה","this"], ["אֲשֶׁר","whose"], ["חַטֹּאתֵיכֶם","sins"], ["נִסְלְחוּ","are forgiven"], ["עַתָּה","now"], ["כִּי","for"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["סוֹלֵחַ","do forgive"], ["חֲטָאִים","sins"], ["וּמְרַחֵם","and am merciful"], ["עַל","unto"], ["הַמִּתְוַדִּים","those who confess"], ["חֲטָאֵיהֶם","their sins"], ["בְּלֵב","with heart"], ["עָנָו׃","humble"]]},
  { num: "ג", words: [["אַךְ","but"], ["אָמֵן","verily"], ["אֲנִי","I"], ["אֹמֵר","say"], ["לָכֶם","unto you"], ["אֵין","it is not"], ["צֹרֶךְ","needful"], ["כִּי","that"], ["כׇל","the whole"], ["חֶבְרַת","company of"], ["זְקֵנַי","my elders"], ["תָּנוּעַ","should be moving"], ["בִּמְהֵרָה","swiftly"], ["עַל","upon"], ["הַמַּיִם","the waters"], ["בְּעוֹד","while"], ["הַיּוֹשְׁבִים","the inhabitants"], ["מִשְּׁנֵי","on the two"], ["עֲבָרֶיהָ","sides"], ["אוֹבְדִים","are perishing"], ["בְּמַעַל׃","in unbelief"]]},
  { num: "ד", words: [
    ["אֲבָל","nevertheless"],
    ["הִנַּחְתִּי","I suffered it"],
    ["לְמַעַן","that"],
    ["תָּעִידוּ","you might bear record"],
    ["הִנֵּה","behold"],
    ["רַבּוֹת","many"],
    ["סַכָּנוֹת","dangers"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["וּבְיֹתֶר","and more especially"],
    ["מֵעַתָּה","from now"],
    ["וָהָלְאָה׃","onward hereafter"]
  ]},
  { num: "ה", words: [
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["גָּזַרְתִּי","have decreed"],
    ["בְאַפִּי","in my anger"],
    ["שַׁמּוֹת","destructions"],
    ["רַבּוֹת","many"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["אָכֵן","yea"],
    ["וּבְיֹתֶר","and especially"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["הָאֵלֶּה׃","these"]
  ]},
  { num: "ו", words: [
    ["אֲבָל","nevertheless"],
    ["כׇּל","all"],
    ["בָּשָׂר","flesh"],
    ["בְּיָדִי","is in my hand"],
    ["וְהַנֶּאֱמָן","and he that is faithful"],
    ["בָּכֶם","among you"],
    ["לֹא","not"],
    ["יֹאבַד","shall perish"],
    ["בַּמָּיִם׃","by the waters"]
  ]},
  { num: "ז", words: [
    ["לָכֵן","therefore"],
    ["נָכוֹן","it is expedient"],
    ["כִּי","that"],
    ["עַבְדִּי","my servant"],
    ["סִידְנִי","Sidney"],
    ["גִילְבֶּרְט","Gilbert"],
    ["וְעַבְדִּי","and my servants"],
    ["וִילְיַם","William"],
    ["וו","W."],
    ["פֶלְפְּס","Phelps"],
    ["יְמַהֲרוּ","be in haste"],
    ["בְּמִשְׁלַחְתָּם","upon their mission"],
    ["וּמִשְׂרָתָם׃","upon their errand and mission"]
  ]},
  { num: "ח", words: [
    ["אֲבָל","nevertheless"],
    ["לֹא","not"],
    ["חָפַצְתִּי","I would suffer"],
    ["כִּי","that"],
    ["תִּפָּרְדוּ","you should part"],
    ["עַד","until"],
    ["תִּוָּסְרוּ","you were chastened"],
    ["עַל","for"],
    ["כׇּל","all"],
    ["חַטֹּאתֵיכֶם","your sins"],
    ["לְמַעַן","that"],
    ["תִּהְיוּ","you might be"],
    ["אֶחָד","one"],
    ["לְמַעַן","that"],
    ["לֹא","not"],
    ["תֹאבְדוּ","you might perish"],
    ["בְּרִשְׁעָה׃","in wickedness"]
  ]},
  { num: "ט", words: [
    ["אַךְ","but"],
    ["עַתָּה","now"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["רָאוּי","it is meet"],
    ["כִּי","that"],
    ["תִּפָּרְדוּ","you should part"],
    ["לָכֵן","wherefore"],
    ["עֲבָדַי","let my servants"],
    ["סִידְנִי","Sidney"],
    ["גִילְבֶּרְט","Gilbert"],
    ["וְוִילְיַם","and William"],
    ["וו","W."],
    ["פֶלְפְּס","Phelps"],
    ["יִקְחוּ","let them take"],
    ["חֶבְרָתָם","their company"],
    ["הָרִאשֹׁנָה","the former"],
    ["וְיִסְעוּ","and let them take"],
    ["לְדַרְכָּם","their journey"],
    ["בִּמְהֵרָה","in haste"],
    ["לְמַלֹּאת","that they may fill"],
    ["מִשְׁלַחְתָּם","their mission"],
    ["וּבֶאֱמוּנָה","and through faith"],
    ["יִגְבָּרוּ׃","they shall overcome"]
  ]},
  { num: "י", words: [
    ["וּכְפִי","and inasmuch"],
    ["אֲשֶׁר","as"],
    ["יִהְיוּ","they shall be"],
    ["נֶאֱמָנִים","faithful"],
    ["יִשָּׁמְרוּ","they shall be preserved"],
    ["וַאֲנִי","and I"],
    ["יְהוָה","the Lord"],
    ["אֶהְיֶה","will be"],
    ["עִמָּם׃","with them"]
  ]},
  { num: "יא", words: [
    ["וְהַשְּׁאָר","and let the residue"],
    ["יִקְחוּ","take"],
    ["הַנָּחוּץ","that which is needful"],
    ["לַמַּלְבּוּשׁ׃","for clothing"]
  ]},
  { num: "יב", words: [
    ["עַבְדִּי","let my servant"],
    ["סִידְנִי","Sidney"],
    ["גִילְבֶּרְט","Gilbert"],
    ["יִקַּח","take"],
    ["עִמּוֹ","with him"],
    ["אֵת־אֲשֶׁר","that which"],
    ["אֵינֶנּוּ","is not"],
    ["נָחוּץ","needful"],
    ["כַּאֲשֶׁר","as"],
    ["תִּוָּעֲדוּ׃","you shall agree"]
  ]},
  { num: "יג", words: [
    ["וְעַתָּה","and now"],
    ["הִנֵּה","behold"],
    ["לְטוֹבַתְכֶם","for your good"],
    ["נָתַתִּי","I gave"],
    ["לָכֶם","unto you"],
    ["מִצְוָה","a commandment"],
    ["עַל","concerning"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["וַאֲנִי","and I"],
    ["יְהוָה","the Lord"],
    ["אוֹכִיחַ","will reason"],
    ["אִתְּכֶם","with you"],
    ["כְּעִם","as with"],
    ["בְּנֵי","the children of"],
    ["אָדָם","men"],
    ["בִּימֵי","in the days of"],
    ["קֶדֶם׃","old"]
  ]},
  { num: "יד", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["בָּרֵאשִׁית","in the beginning"],
    ["בֵּרַכְתִּי","blessed"],
    ["הַמַּיִם","the waters"],
    ["אַךְ","but"],
    ["בְּאַחֲרִית","in the latter part of"],
    ["הַיָּמִים","the days"],
    ["בְּפִי","by the mouth of"],
    ["עַבְדִּי","my servant"],
    ["יוֹחָנָן","John"],
    ["קִלַּלְתִּי","I cursed"],
    ["הַמָּיִם׃","the waters"]
  ]},
  { num: "טו", words: [
    ["לָכֵן","wherefore"],
    ["יָבֹאוּ","shall come"],
    ["יָמִים","days"],
    ["אֲשֶׁר","when"],
    ["לֹא","not"],
    ["יִהְיֶה","shall be"],
    ["בָשָׂר","flesh"],
    ["בָּטוּחַ","safe"],
    ["עַל","upon"],
    ["הַמָּיִם׃","the waters"]
  ]},
  { num: "טז", words: [
    ["וְיֵאָמֵר","and it shall be said"],
    ["בַּיָּמִים","in the days"],
    ["הַבָּאִים","that are to come"],
    ["כִּי","that"],
    ["אֵין","none"],
    ["יָכוֹל","is able"],
    ["לַעֲלוֹת","to go up"],
    ["לְאֶרֶץ","to the land of"],
    ["צִיּוֹן","Zion"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["כִּי","but"],
    ["אִם","only"],
    ["יְשַׁר","the upright of"],
    ["לֵב׃","heart"]
  ]},
  { num: "יז", words: [
    ["וְכַאֲשֶׁר","and as"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["בָּרֵאשִׁית","in the beginning"],
    ["קִלַּלְתִּי","cursed"],
    ["הָאָרֶץ","the land"],
    ["כֵּן","even so"],
    ["בְּאַחֲרִית","in the latter part of"],
    ["הַיָּמִים","the days"],
    ["בֵּרַכְתִּיהָ","I have blessed it"],
    ["בְּעִתָּהּ","in its time"],
    ["לַעֲבֹדַת","for the use of"],
    ["קְדוֹשַׁי","my saints"],
    ["לְמַעַן","that"],
    ["יֹאכְלוּ","they may partake"],
    ["מֵחֶלְבָּהּ׃","of its fatness"]
  ]},
  { num: "יח", words: [
    ["וְעַתָּה","and now"],
    ["אֲנִי","I"],
    ["נוֹתֵן","give"],
    ["לָכֶם","unto you"],
    ["מִצְוָה","a commandment"],
    ["כִּי","that"],
    ["אֲשֶׁר","what"],
    ["אֹמַר","I say"],
    ["לְאֶחָד","unto one"],
    ["אֹמֵר","I say"],
    ["לַכֹּל","unto all"],
    ["תַּזְהִירוּ","you shall forewarn"],
    ["אֲחֵיכֶם","your brethren"],
    ["עַל","concerning"],
    ["הַמַּיִם","the waters"],
    ["הָאֵלֶּה","these"],
    ["אֲשֶׁר","that"],
    ["לֹא","not"],
    ["יָבֹאוּ","they come"],
    ["לִנְסֹעַ","to journey"],
    ["עֲלֵיהֶם","upon them"],
    ["פֶּן","lest"],
    ["תִּכָּשֵׁל","fail"],
    ["אֱמוּנָתָם","their faith"],
    ["וְיִלָּכְדוּ","and they be caught"],
    ["בַּפַּחִים׃","in snares"]
  ]},
  { num: "יט", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["גָּזַרְתִּי","have decreed"],
    ["וְהַמַּשְׁחִית","and the destroyer"],
    ["רֹכֵב","rides"],
    ["עַל","upon"],
    ["פְּנֵיהֶם","the face of them"],
    ["וְאֵינֶנִּי","and I do not"],
    ["מְבַטֵּל","revoke"],
    ["הַגְּזֵרָה׃","the decree"]
  ]},
  { num: "כ", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["כָּעַסְתִּי","was angry"],
    ["עֲלֵיכֶם","with you"],
    ["אֶתְמוֹל","yesterday"],
    ["אַךְ","but"],
    ["הַיּוֹם","today"],
    ["שָׁב","is turned away"],
    ["אַפִּי׃","my anger"]
  ]},
  { num: "כא", words: [
    ["לָכֵן","wherefore"],
    ["אֵלֶּה","those"],
    ["אֲשֶׁר","concerning whom"],
    ["דִּבַּרְתִּי","I have spoken"],
    ["עֲלֵיהֶם","concerning them"],
    ["כִּי","that"],
    ["יִסְעוּ","should take"],
    ["לְדַרְכָּם","their journey"],
    ["בִּמְהֵרָה","in haste"],
    ["עוֹד","again"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["יִסְעוּ","let them take"],
    ["לְדַרְכָּם","their journey"],
    ["בִּמְהֵרָה׃","in haste"]
  ]},
  { num: "כב", words: [
    ["וְאַחַת","and it matters"],
    ["הִיא","not"],
    ["לִי","unto me"],
    ["אַחֲרֵי","after"],
    ["מְעַט","a little while"],
    ["אִם","if"],
    ["יְמַלְּאוּ","they fill"],
    ["מִשְׁלַחְתָּם","their mission"],
    ["אִם","whether"],
    ["יֵלְכוּ","they go"],
    ["בַמַּיִם","by water"],
    ["אוֹ","or"],
    ["בַיַּבָּשָׁה","by land"],
    ["יְהִי","let be"],
    ["זֹאת","this"],
    ["כַּאֲשֶׁר","as"],
    ["יִוָּדַע","shall be made known"],
    ["לָהֶם","unto them"],
    ["לְפִי","according to"],
    ["מִשְׁפָּטָם","their judgments"],
    ["אַחֲרֵי","after"],
    ["כֵן׃","this hereafter"]
  ]},
  { num: "כג", words: [
    ["וְעַתָּה","and now"],
    ["עַל","concerning"],
    ["עֲבָדַי","my servants"],
    ["סִידְנִי","Sidney"],
    ["רִיגְדּוֹן","Rigdon"],
    ["וְיוֹסֵף","and Joseph"],
    ["סְמִית","Smith"],
    ["הַבֵּן","Jun."],
    ["וְאוֹלִיבֶר","and Oliver"],
    ["קַאוּדְרִי","Cowdery"],
    ["לֹא","not"],
    ["יָבֹאוּ","let them come"],
    ["עוֹד","again"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["כִּי","save"],
    ["אִם",""],
    ["עַל","upon"],
    ["הַתְּעָלָה","the canal"],
    ["בִּנְסֹעָם","while journeying"],
    ["לְבָתֵּיהֶם","unto their homes"],
    ["אוֹ","or"],
    ["בְּמִלִּים","in words"],
    ["אֲחֵרוֹת","other"],
    ["לֹא","not"],
    ["יָבֹאוּ","they shall come"],
    ["עַל","upon"],
    ["הַמַּיִם","the waters"],
    ["לִנְסֹעַ","to journey"],
    ["כִּי","save"],
    ["אִם",""],
    ["עַל","upon"],
    ["הַתְּעָלָה׃","the canal"]
  ]},
  { num: "כד", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מִנִּיתִי","have appointed"],
    ["דֶּרֶךְ","a way"],
    ["לְמַסַּע","for the journeying of"],
    ["קְדוֹשָׁי","my saints"],
    ["וְהִנֵּה","and behold"],
    ["זֶה","this is"],
    ["הַדֶּרֶךְ","the way"],
    ["אַחֲרֵי","after"],
    ["עָזְבָם","their leaving"],
    ["הַתְּעָלָה","the canal"],
    ["יִסְעוּ","they shall journey"],
    ["בַיַּבָּשָׁה","by land"],
    ["כַּאֲשֶׁר","inasmuch as"],
    ["מְצֻוִּים","they are commanded"],
    ["לִנְסֹעַ","to journey"],
    ["וְלַעֲלוֹת","and to go up"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "כה", words: [
    ["וְיַעֲשׂוּ","and they shall do"],
    ["כִּבְנֵי","like the children of"],
    ["יִשְׂרָאֵל","Israel"],
    ["וְיִטְּעוּ","pitching"],
    ["אׇהֳלֵיהֶם","their tents"],
    ["בַּדָּרֶךְ׃","by the way"]
  ]},
  { num: "כו", words: [
    ["וְהִנֵּה","and behold"],
    ["הַמִּצְוָה","the commandment"],
    ["הַזֹּאת","this"],
    ["תִּתְּנוּ","you shall give"],
    ["לְכׇל","unto all"],
    ["אֲחֵיכֶם׃","your brethren"]
  ]},
  { num: "כז", words: [
    ["אֲבָל","nevertheless"],
    ["לַאֲשֶׁר","unto whom"],
    ["נִתַּן","is given"],
    ["כֹּחַ","power"],
    ["לִגְעֹר","to rebuke"],
    ["בַּמַּיִם","the waters"],
    ["לוֹ","unto him"],
    ["נִתָּן","is given"],
    ["מִן","by"],
    ["הָרוּחַ","the Spirit"],
    ["לָדַעַת","to know"],
    ["כׇּל","all"],
    ["דְּרָכָיו׃","his ways"]
  ]},
  { num: "כח", words: [
    ["לָכֵן","wherefore"],
    ["יַעֲשֶׂה","let him do"],
    ["כַּאֲשֶׁר","as"],
    ["תְּצַוֶּנּוּ","shall command him"],
    ["רוּחַ","the Spirit of"],
    ["אֱלֹהִים","God"],
    ["חַיִּים","the living"],
    ["אִם","whether"],
    ["בַּיַּבָּשָׁה","upon the land"],
    ["וְאִם","or"],
    ["בַּמַּיִם","upon the waters"],
    ["כַּאֲשֶׁר","as"],
    ["יִשָּׁאֵר","it remains"],
    ["לִי","with me"],
    ["לַעֲשׂוֹת","to do"],
    ["אַחֲרֵי","after"],
    ["כֵן׃","this"]
  ]},
  { num: "כט", words: [
    ["וְלָכֶם","and unto you"],
    ["נִתַּן","is given"],
    ["הַמַּסְלוּל","the course"],
    ["לַקְּדוֹשִׁים","for the saints"],
    ["אוֹ","or"],
    ["הַדֶּרֶךְ","the way"],
    ["לִקְדוֹשֵׁי","for the saints of"],
    ["מַחֲנֵה","the camp of"],
    ["יְהוָה","the Lord"],
    ["לִנְסֹעַ׃","to journey"]
  ]},
  { num: "ל", words: [
    ["וְעוֹד","and again"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["עֲבָדַי","my servants"],
    ["סִידְנִי","Sidney"],
    ["רִיגְדּוֹן","Rigdon"],
    ["וְיוֹסֵף","and Joseph"],
    ["סְמִית","Smith"],
    ["הַבֵּן","Jun."],
    ["וְאוֹלִיבֶר","and Oliver"],
    ["קַאוּדְרִי","Cowdery"],
    ["לֹא","not"],
    ["יִפְתְּחוּ","shall open"],
    ["פִּיהֶם","their mouths"],
    ["בְּקֶרֶב","in the midst of"],
    ["קְהִלּוֹת","the congregations of"],
    ["הָרְשָׁעִים","the wicked"],
    ["עַד","until"],
    ["בֹּאָם","their arriving"],
    ["לְסִינְסִינֶּאטִי׃","at Cincinnati"]
  ]},
  { num: "לא", words: [
    ["וּבַמָּקוֹם","and in the place"],
    ["הַהוּא","that"],
    ["יִשְׂאוּ","they shall lift up"],
    ["קוֹלָם","their voices"],
    ["לֵאלֹהִים","unto God"],
    ["עַל","against"],
    ["הָעָם","the people"],
    ["הַהוּא","that"],
    ["אָכֵן","yea"],
    ["אֵלָיו","unto him"],
    ["אֲשֶׁר","whose"],
    ["בָּעֵר","burns"],
    ["אַפּוֹ","anger"],
    ["בְּרִשְׁעָתָם","against their wickedness"],
    ["עַם","a people"],
    ["קָרוֹב","are near"],
    ["לְהִבָּשֵׁל","to be ripened"],
    ["לְשַׁמָּה׃","for destruction"]
  ]},
  { num: "לב", words: [
    ["וּמִשָּׁם","and from thence"],
    ["יִסְעוּ","let them journey"],
    ["לְדַרְכָּם","their way"],
    ["לְעֵדוֹת","to the congregations of"],
    ["אֲחֵיהֶם","their brethren"],
    ["כִּי","for"],
    ["עֲמָלָם","their labors"],
    ["נָחוּץ","are needed"],
    ["עַתָּה","now"],
    ["יוֹתֵר","more"],
    ["בֵּינֵיהֶם","among them"],
    ["מִבֵּין","than among"],
    ["קְהִלּוֹת","the congregations of"],
    ["הָרְשָׁעִים׃","the wicked"]
  ]},
  { num: "לג", words: [
    ["וְעַתָּה","and now"],
    ["עַל","concerning"],
    ["הַשְּׁאָר","the residue"],
    ["יִסְעוּ","let them journey"],
    ["וְיַכְרִיזוּ","and declare"],
    ["הַדָּבָר","the word"],
    ["בְּקֶרֶב","among"],
    ["קְהִלּוֹת","the congregations of"],
    ["הָרְשָׁעִים","the wicked"],
    ["כַּאֲשֶׁר","inasmuch as"],
    ["נִתָּן׃","it is given"]
  ]},
  { num: "לד", words: [
    ["וּכְפִי","and inasmuch"],
    ["אֲשֶׁר","as"],
    ["יַעֲשׂוּ","they do"],
    ["זֹאת","this"],
    ["יְנַקּוּ","they shall rid"],
    ["בִגְדֵיהֶם","their garments"],
    ["וְיִהְיוּ","and they shall be"],
    ["נְקִיִּים","spotless"],
    ["לְפָנָי׃","before me"]
  ]},
  { num: "לה", words: [
    ["וְיִסְעוּ","and let them journey"],
    ["יַחְדָּו","together"],
    ["אוֹ","or"],
    ["שְׁנַיִם","two"],
    ["שְׁנַיִם","by two"],
    ["כַּטּוֹב","as is good"],
    ["בְּעֵינֵיהֶם","in their eyes"],
    ["רַק","only"],
    ["עַבְדִּי","let my servant"],
    ["רֵינוֹלְדְּס","Reynolds"],
    ["קָהוּן","Cahoon"],
    ["וְעַבְדִּי","and my servant"],
    ["שְׁמוּאֵל","Samuel"],
    ["ה.","H."],
    ["סְמִית","Smith"],
    ["אֲשֶׁר","with whom"],
    ["רָצוּי","I am pleased"],
    ["בָם","with them"],
    ["לֹא","not"],
    ["יִפָּרְדוּ","be separated"],
    ["עַד","until"],
    ["שׁוּבָם","their returning"],
    ["לְבָתֵּיהֶם","to their homes"],
    ["וְזֹאת","and this"],
    ["לְתַכְלִית","for a purpose"],
    ["חֲכָמָה","wise"],
    ["בִּי׃","in me"]
  ]},
  { num: "לו", words: [
    ["וְעַתָּה","and now"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["וַאֲשֶׁר","and what"],
    ["אֹמֵר","I say"],
    ["לְאֶחָד","unto one"],
    ["אֹמֵר","I say"],
    ["לַכֹּל","unto all"],
    ["חִזְקוּ","be of good cheer"],
    ["וְאִמְצוּ","be strong"],
    ["יְלָדִים","children"],
    ["קְטַנִּים","little"],
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["בְתוֹכְכֶם","am in your midst"],
    ["וְלֹא","and not"],
    ["עֲזַבְתִּיכֶם׃","have I forsaken you"]
  ]},
  { num: "לז", words: [
    ["וּכְפִי","and inasmuch as"],
    ["אֲשֶׁר","that"],
    ["נִכְנַעְתֶּם","you have humbled yourselves"],
    ["לְפָנַי","before me"],
    ["בִּרְכוֹת","the blessings of"],
    ["הַמַּלְכוּת","the kingdom"],
    ["לָכֶם","unto you"],
    ["הֵנָּה׃","behold"]
  ]},
  { num: "לח", words: [
    ["אִזְרוּ","gird up"],
    ["מׇתְנֵיכֶם","your loins"],
    ["וֶהְיוּ","and be"],
    ["עֵרִים","watchful"],
    ["וּצְנוּעִים","and be sober"],
    ["וְצַפּוּ","and look forth"],
    ["לְבוֹא","for the coming of"],
    ["בֶן","the Son of"],
    ["הָאָדָם","Man"],
    ["כִּי","for"],
    ["בָא","he comes"],
    ["בְשָׁעָה","in an hour"],
    ["אֲשֶׁר","which"],
    ["לֹא","not"],
    ["תַחְשֹׁבוּ׃","you think"]
  ]},
  { num: "לט", words: [
    ["הִתְפַּלְּלוּ","pray"],
    ["תָמִיד","always"],
    ["לְבַל","that not"],
    ["תָּבֹאוּ","you enter"],
    ["לְנִסָּיוֹן","into temptation"],
    ["לְמַעַן","that"],
    ["תַּעַמְדוּ","you may abide"],
    ["בְּיוֹם","in the day"],
    ["בֹּאוֹ","of his coming"],
    ["אִם","whether"],
    ["בַּחַיִּים","in life"],
    ["וְאִם","or"],
    ["בַּמָּוֶת","in death"],
    ["כֵּן","even so"],
    ["יְהִי","may it be"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc61_ch1Verses, 'dc61-ch1-verses');



var dc62_ch1Verses = [
  { num: "א", words: [["הִנֵּה","behold"], ["וְהַאֲזִינוּ","and hearken"], ["זִקְנֵי","O elders of"], ["כְּנֵסִיָּתִי","my church"], ["אָמַר","says"], ["יְהוָה","the Lord"], ["אֱלֹהֵיכֶם","your God"], ["יֵשׁוּעַ","Jesus"], ["הַמָּשִׁיחַ","Christ"], ["מְלִיצְכֶם","your advocate"], ["הַיּוֹדֵעַ","who knows"], ["חֻלְשַׁת","the weakness of"], ["הָאָדָם","man"], ["וְאֵיךְ","and how"], ["לְסַעֵד","to succor"], ["הַמְּנֻסִּים׃","them who are tempted"]]},
  { num: "ב", words: [
    ["וְאָמֵן","and verily"],
    ["עֵינַי","my eyes"],
    ["עַל","are upon"],
    ["אֵלֶּה","those"],
    ["אֲשֶׁר","who"],
    ["עוֹד","as yet"],
    ["לֹא","not"],
    ["עָלוּ","have gone up"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["לָכֵן","wherefore"],
    ["מִשְׁלַחְתְּכֶם","your mission"],
    ["עוֹד","yet"],
    ["לֹא","not"],
    ["נִשְׁלְמָה׃","is full"]
  ]},
  { num: "ג", words: [
    ["אֲבָל","nevertheless"],
    ["אַשְׁרֵיכֶם","blessed are you"],
    ["כִּי","for"],
    ["הָעֵדוּת","the testimony"],
    ["אֲשֶׁר","which"],
    ["הָעַדְתֶּם","you have borne"],
    ["רְשׁוּמָה","is recorded"],
    ["בַשָּׁמַיִם","in heaven"],
    ["לַמַּלְאָכִים","for the angels"],
    ["לְהַבִּיט","to look"],
    ["בָּהּ","upon it"],
    ["וְהֵם","and they"],
    ["שְׂמֵחִים","rejoice"],
    ["עֲלֵיכֶם","over you"],
    ["וְחַטֹּאתֵיכֶם","and your sins"],
    ["נִסְלְחוּ","are forgiven"],
    ["לָכֶם׃","unto you"]
  ]},
  { num: "ד", words: [
    ["וְעַתָּה","and now"],
    ["הוֹסִיפוּ","continue"],
    ["מַסַּעֲכֶם","your journey"],
    ["הֵאָסְפוּ","assemble yourselves"],
    ["בְאֶרֶץ","upon the land of"],
    ["צִיּוֹן","Zion"],
    ["וַעֲשׂוּ","and hold"],
    ["אֲסֵפָה","a meeting"],
    ["וְשִׂמְחוּ","and rejoice"],
    ["יַחְדָּו","together"],
    ["וְהַקְרִיבוּ","and offer"],
    ["קָרְבָּן","a sacrament"],
    ["לָעֶלְיוֹן׃","unto the Most High"]
  ]},
  { num: "ה", words: [
    ["וְאָז","and then"],
    ["תּוּכְלוּ","you may"],
    ["לָשׁוּב","return"],
    ["לְהָעִיד","to bear record"],
    ["אָכֵן","yea"],
    ["כֻּלְּכֶם","altogether"],
    ["יַחְדָּו","together"],
    ["אוֹ","or"],
    ["שְׁנַיִם","two"],
    ["שְׁנַיִם","by two"],
    ["כַּטּוֹב","as is good"],
    ["בְּעֵינֵיכֶם","in your eyes"],
    ["אַחַת","it matters"],
    ["הִיא","not"],
    ["לִי","unto me"],
    ["רַק","only"],
    ["הֱיוּ","be"],
    ["נֶאֱמָנִים","faithful"],
    ["וְהַכְרִיזוּ","and declare"],
    ["בְּשׂוֹרָה","tidings"],
    ["טוֹבָה","glad"],
    ["לְיוֹשְׁבֵי","unto the inhabitants of"],
    ["הָאָרֶץ","the earth"],
    ["אוֹ","or"],
    ["בְּקֶרֶב","among"],
    ["קְהִלּוֹת","the congregations of"],
    ["הָרְשָׁעִים׃","the wicked"]
  ]},
  { num: "ו", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["קִבַּצְתִּיכֶם","have brought you together"],
    ["לְמַעַן","that"],
    ["תָּקוּם","might be fulfilled"],
    ["הַהַבְטָחָה","the promise"],
    ["כִּי","that"],
    ["הַנֶּאֱמָנִים","the faithful"],
    ["בָּכֶם","among you"],
    ["יִשָּׁמְרוּ","should be preserved"],
    ["וְיִשְׂמְחוּ","and rejoice"],
    ["יַחְדָּו","together"],
    ["בְּאֶרֶץ","in the land of"],
    ["מִיזוּרִי","Missouri"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מַבְטִיחַ","promise"],
    ["לַנֶּאֱמָנִים","to the faithful"],
    ["וְאֵינֶנִּי","and I am not"],
    ["יָכוֹל","able"],
    ["לְשַׁקֵּר׃","to lie"]
  ]},
  { num: "ז", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["חָפֵץ","am willing"],
    ["אִם","if"],
    ["יֵשׁ","there is"],
    ["בָּכֶם","among you"],
    ["הֶחָפֵץ","one who desires"],
    ["לִרְכֹּב","to ride"],
    ["עַל","upon"],
    ["סוּסִים","horses"],
    ["אוֹ","or"],
    ["עַל","upon"],
    ["פְּרָדִים","mules"],
    ["אוֹ","or"],
    ["בְּמֶרְכָּבוֹת","in chariots"],
    ["יְקַבֵּל","he shall receive"],
    ["הַבְּרָכָה","the blessing"],
    ["הַזֹּאת","this"],
    ["אִם","if"],
    ["יְקַבֵּל","he receive"],
    ["אוֹתָהּ","it"],
    ["מִיַּד","from the hand of"],
    ["יְהוָה","the Lord"],
    ["בְּלֵב","with heart"],
    ["מוֹדֶה","thankful"],
    ["בְּכׇל","in all"],
    ["דָּבָר׃","things"]
  ]},
  { num: "ח", words: [
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["נִשְׁאָרִים","remain"],
    ["לָכֶם","unto you"],
    ["לַעֲשׂוֹת","to do"],
    ["לְפִי","according to"],
    ["מִשְׁפָּט","judgment"],
    ["וְתוֹרֹת","and the directions of"],
    ["הָרוּחַ׃","the Spirit"]
  ]},
  { num: "ט", words: [
    ["הִנֵּה","behold"],
    ["הַמַּלְכוּת","the kingdom"],
    ["לָכֶם","is yours"],
    ["וְהִנֵּה","and behold"],
    ["אֲנִי","I"],
    ["עִם","am with"],
    ["הַנֶּאֱמָנִים","the faithful"],
    ["תָּמִיד","always"],
    ["כֵּן","even so"],
    ["יְהִי","may it be"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc62_ch1Verses, 'dc62-ch1-verses');



var dc63_ch1Verses = [
  { num: "א", words: [
    ["הַאֲזִינוּ","hearken"],
    ["הָעָם","O people"],
    ["וּפִתְחוּ","and open"],
    ["לְבַבְכֶם","your hearts"],
    ["וְהַטּוּ","and give"],
    ["אֹזֶן","ear"],
    ["מֵרָחוֹק","from afar"],
    ["וְהַקְשִׁיבוּ","and listen"],
    ["אַתֶּם","you"],
    ["הַקּוֹרְאִים","that call"],
    ["לְנַפְשְׁכֶם","yourselves"],
    ["עַם","the people of"],
    ["יְהוָה","the Lord"],
    ["וְשִׁמְעוּ","and hear"],
    ["דְּבַר","the word of"],
    ["יְהוָה","the Lord"],
    ["וּרְצוֹנוֹ","and his will"],
    ["עֲלֵיכֶם׃","concerning you"]
  ]},
  { num: "ב", words: [
    ["אָכֵן","yea"],
    ["אָמֵן","verily"],
    ["שִׁמְעוּ","hear"],
    ["דְּבַר","the word of"],
    ["אֲשֶׁר","him whose"],
    ["בָּעֵר","burns"],
    ["אַפּוֹ","anger"],
    ["בָּרְשָׁעִים׃","against the wicked"]
  ]},
  { num: "ג", words: [
    ["הֶחָפֵץ","who is willing"],
    ["לָקַחַת","to take"],
    ["אֲשֶׁר","whom"],
    ["יַחְפֹּץ","he will"],
    ["לָקַחַת","to take"],
    ["וּמְחַיֶּה","and preserves in life"],
    ["אֲשֶׁר","whom"],
    ["יַחְפֹּץ","he will"],
    ["לְחַיּוֹת׃","to preserve"]
  ]},
  { num: "ד", words: [
    ["הַבּוֹנֶה","who builds"],
    ["לְפִי","according to"],
    ["רְצוֹנוֹ","his will"],
    ["וַחֲפָצוֹ","and pleasure"],
    ["וּמַשְׁחִית","and destroys"],
    ["כַּחֲפָצוֹ","as he pleases"],
    ["וְיָכוֹל","and is able"],
    ["לְהַשְׁלִיךְ","to cast"],
    ["הַנֶּפֶשׁ","the soul"],
    ["לְגֵיהִנֹּם׃","down to hell"]
  ]},
  { num: "ה", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["נוֹתֵן","utter"],
    ["קוֹלִי","my voice"],
    ["וְיִשָּׁמַע׃","and it shall be obeyed"]
  ]},
  { num: "ו", words: [
    ["לָכֵן","wherefore"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["יִשָּׁמְרוּ","let take heed"],
    ["הָרְשָׁעִים","the wicked"],
    ["וְיִירְאוּ","and let fear"],
    ["הַמּוֹרְדִים","the rebellious"],
    ["וְיֶחֱרְדוּ","and tremble"],
    ["וְיֶחֱשׁוּ","and let hold their lips"],
    ["חַסְרֵי","those lacking"],
    ["הָאֱמוּנָה","faith"],
    ["כִּי","for"],
    ["יוֹם","the day of"],
    ["הַזַּעַם","wrath"],
    ["יָבוֹא","shall come"],
    ["עֲלֵיהֶם","upon them"],
    ["כַּסּוּפָה","as a whirlwind"],
    ["וְיֵדַע","and shall know"],
    ["כׇּל","all"],
    ["בָּשָׂר","flesh"],
    ["כִּי","that"],
    ["אֲנִי","I am"],
    ["אֱלֹהִים׃","God"]
  ]},
  { num: "ז", words: [
    ["וְהַמְבַקֵּשׁ","and he that seeks"],
    ["אוֹתוֹת","signs"],
    ["יִרְאֶה","shall see"],
    ["אוֹתוֹת","signs"],
    ["אַךְ","but"],
    ["לֹא","not"],
    ["לִישׁוּעָה׃","unto salvation"]
  ]},
  { num: "ח", words: [
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["יֵשׁ","there are"],
    ["בָּכֶם","among you"],
    ["הַמְבַקְּשִׁים","those who seek"],
    ["אוֹתוֹת","signs"],
    ["וְהָיוּ","and there have been"],
    ["כָאֵלֶּה","such"],
    ["מֵרֵאשִׁית׃","from the beginning"]
  ]},
  { num: "ט", words: [["אַךְ","but"], ["הִנֵּה","behold"], ["אֵין","not"], ["אֱמוּנָה","faith"], ["בָאָה","comes"], ["מֵאוֹתוֹת","by signs"], ["כִּי־אִם","but"], ["אוֹתוֹת","signs"], ["הוֹלְכִים","follow"], ["אַחֲרֵי","after"], ["הַמַּאֲמִינִים׃","those who believe"]]},
  { num: "י", words: [["אָכֵן","yea"], ["אוֹתוֹת","signs"], ["בָּאִים","come"], ["מֵאֱמוּנָה","by faith"], ["לֹא","not"], ["מֵרְצוֹן","by the will of"], ["בְּנֵי","the children of"], ["אָדָם","men"], ["וְלֹא","nor"], ["כַּחֲפָצָם","as they please"], ["כִּי־אִם","but"], ["מֵרְצוֹן","by the will of"], ["אֱלֹהִים׃","God"]]},
  { num: "יא", words: [
    ["אָכֵן","yea"],
    ["אוֹתוֹת","signs"],
    ["בָּאִים","come"],
    ["מֵאֱמוּנָה","by faith"],
    ["לְמַעֲשִׂים","unto works"],
    ["גְּדוֹלִים","mighty"],
    ["כִּי","for"],
    ["בְלֹא","without"],
    ["אֱמוּנָה","faith"],
    ["אֵין","no"],
    ["אָדָם","man"],
    ["מוֹצֵא","finds"],
    ["חֵן","favor"],
    ["בְּעֵינֵי","in the eyes of"],
    ["אֱלֹהִים","God"],
    ["וַאֲשֶׁר","and he with whom"],
    ["כּוֹעֵס","is angry"],
    ["עָלָיו","with him"],
    ["אֱלֹהִים","God"],
    ["אֵינֶנּוּ","is not"],
    ["רָצוּי","pleased"],
    ["לָכֵן","wherefore"],
    ["לָהֶם","unto them"],
    ["אֵינֶנּוּ","he does not"],
    ["מַרְאֶה","show"],
    ["אוֹתוֹת","signs"],
    ["רַק","only"],
    ["בְּחֵמָה׃","in wrath"]
  ]},
  { num: "יב", words: [
    ["לָכֵן","wherefore"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אֵינֶנִּי","am not"],
    ["רָצוּי","pleased"],
    ["בָּאֵלֶּה","with those"],
    ["אֲשֶׁר","who"],
    ["בִּקְּשׁוּ","have sought after"],
    ["אוֹתוֹת","signs"],
    ["וּמוֹפְתִים","and wonders"],
    ["לֶאֱמוּנָה","not for faith"],
    ["וְלֹא","and not"],
    ["לְטוֹבַת","for the good of"],
    ["בְּנֵי","the children of"],
    ["אָדָם","men"],
    ["לִכְבוֹדִי׃","unto my glory"]
  ]},
  { num: "יג", words: [
    ["אֲבָל","nevertheless"],
    ["אֲנִי","I"],
    ["נוֹתֵן","I give"],
    ["מִצְוֹת","commandments"],
    ["וְרַבִּים","and many"],
    ["פָּנוּ","have turned away"],
    ["מִמִּצְוֹתַי","from my commandments"],
    ["וְלֹא","and not"],
    ["שְׁמָרוּם׃","kept them"]
  ]},
  { num: "יד", words: [
    ["הָיוּ","there were"],
    ["בֵינֵיכֶם","among you"],
    ["נֹאֲפִים","adulterers"],
    ["וְנֹאֲפוֹת","and adulteresses"],
    ["מֵהֶם","some of whom"],
    ["פָּנוּ","have turned away"],
    ["מִכֶּם","from you"],
    ["וַאֲחֵרִים","and others"],
    ["נִשְׁאָרִים","remain"],
    ["אִתְּכֶם","with you"],
    ["אֲשֶׁר","that"],
    ["יִגָּלוּ","shall be revealed"],
    ["אַחֲרֵי","after"],
    ["כֵן׃","this hereafter"]
  ]},
  { num: "טו", words: [
    ["אֵלֶּה","let such"],
    ["יִשָּׁמְרוּ","beware"],
    ["וְיָשׁוּבוּ","and repent"],
    ["בִּמְהֵרָה","speedily"],
    ["פֶּן","lest"],
    ["יָבוֹא","shall come"],
    ["עֲלֵיהֶם","upon them"],
    ["הַמִּשְׁפָּט","judgment"],
    ["כַּפַּח","as a snare"],
    ["וְאִוַּלְתָּם","and their folly"],
    ["תִּגָּלֶה","shall be made manifest"],
    ["וּמַעֲשֵׂיהֶם","and their works"],
    ["יֵלְכוּ","shall follow"],
    ["אַחֲרֵיהֶם","after them"],
    ["לְעֵינֵי","in the eyes of"],
    ["הָעָם׃","the people"]
  ]},
  { num: "טז", words: [["וְאָמֵן","and verily"], ["אֲנִי","I"], ["אֹמֵר","say"], ["לָכֶם","unto you"], ["כַּאֲשֶׁר","as"], ["אָמַרְתִּי","I have said"], ["לִפְנֵי","before"], ["הַמַּבִּיט","he that looks"], ["עַל","upon"], ["אִשָּׁה","a woman"], ["לְחָמְדָהּ","to lust after her"], ["אוֹ","or"], ["אִם","if any"], ["יְנָאֲפוּ","shall commit adultery"], ["בְּלִבָּם","in their hearts"], ["לֹא","not"], ["יִהְיֶה","shall have"], ["לָהֶם","unto them"], ["הָרוּחַ","the Spirit"], ["כִּי־אִם","but"], ["יְכַחֲשׁוּ","they shall deny"], ["בָאֱמוּנָה","the faith"], ["וְיִפְחָדוּ׃","and shall fear"]]},
  { num: "יז", words: [["לָכֵן","therefore"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["אָמַרְתִּי","have said"], ["כִּי","for"], ["רַכֵּי־הַלֵּב","the faint of heart"], ["וְחַסְרֵי","that those lacking"], ["הָאֱמוּנָה","the faith"], ["וְכׇל","and all"], ["הַמְכַזְּבִים","the liars"], ["וְכׇל","and every"], ["אוֹהֵב","one who loves"], ["וְעוֹשֶׂה","and makes"], ["שֶׁקֶר","a lie"], ["וְהַזּוֹנֶה","and the whoremonger"], ["וְהַמְכַשֵּׁף","and the sorcerer"], ["חֶלְקָם","shall have their part"], ["בַּאֲגָם","in the lake"], ["הַבּוֹעֵר","that burns"], ["בְּאֵשׁ","with fire"], ["וְגָפְרִית","and brimstone"], ["אֲשֶׁר","which"], ["הוּא","is"], ["הַמָּוֶת","the death"], ["הַשֵּׁנִי׃","the second"]]},
  { num: "יח", words: [
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לֹא","not"],
    ["יִהְיֶה","shall be"],
    ["לָהֶם","unto them"],
    ["חֵלֶק","part"],
    ["בַּתְּחִיָּה","in the resurrection"],
    ["הָרִאשׁוֹנָה׃","the first"]
  ]},
  { num: "יט", words: [
    ["וְעַתָּה","and now"],
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["אֵינְכֶם","you are not"],
    ["נִצְדָּקִים","justified"],
    ["כִּי","because"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["בֵּינֵיכֶם׃","are among you"]
  ]},
  { num: "כ", words: [
    ["אֲבָל","nevertheless"],
    ["הָעוֹמֵד","he that endures"],
    ["בֶּאֱמוּנָה","in faith"],
    ["וְעֹשֶׂה","and does"],
    ["רְצוֹנִי","my will"],
    ["יִגְבַּר","the same shall overcome"],
    ["וִיקַבֵּל","and shall receive"],
    ["נַחֲלָה","an inheritance"],
    ["עַל","upon"],
    ["הָאָרֶץ","the earth"],
    ["בְּבוֹא","when shall come"],
    ["יוֹם","the day of"],
    ["הַתְּמוּרָה׃","transfiguration"]
  ]},
  { num: "כא", words: [
    ["כַּאֲשֶׁר","when"],
    ["תְּשֻׁנֶּה","shall be transfigured"],
    ["הָאָרֶץ","the earth"],
    ["לְפִי","according to"],
    ["הַתַּבְנִית","the pattern"],
    ["אֲשֶׁר","which"],
    ["הָרְאָתָה","was shown"],
    ["לִשְׁלוּחַי","unto my apostles"],
    ["עַל","upon"],
    ["הָהָר","the mount"],
    ["אֲשֶׁר","of which"],
    ["מְלֹאוֹ","the fulness"],
    ["עוֹד","yet"],
    ["לֹא","not"],
    ["קִבַּלְתֶּם׃","you have received"]
  ]},
  { num: "כב", words: [
    ["וְעַתָּה","and now"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["כַּאֲשֶׁר","as"],
    ["אָמַרְתִּי","I said"],
    ["כִּי","that"],
    ["אוֹדִיעַ","I would make known"],
    ["רְצוֹנִי","my will"],
    ["לָכֶם","unto you"],
    ["הִנֵּה","behold"],
    ["אוֹדִיעֶנּוּ","I will make it known"],
    ["לָכֶם","unto you"],
    ["לֹא","not"],
    ["בְּדֶרֶךְ","by way of"],
    ["מִצְוָה","commandment"],
    ["כִּי","for"],
    ["רַבִּים","many"],
    ["אֵינָם","there are who do not"],
    ["שֹׁמְרִים","observe"],
    ["לִשְׁמֹר","to keep"],
    ["מִצְוֹתָי׃","my commandments"]
  ]},
  { num: "כג", words: [
    ["אַךְ","but"],
    ["לַשֹּׁמֵר","unto him that keeps"],
    ["מִצְוֹתַי","my commandments"],
    ["אֶתֵּן","I will give"],
    ["סוֹדוֹת","the mysteries of"],
    ["מַלְכוּתִי","my kingdom"],
    ["וְהֵם","and they"],
    ["יִהְיוּ","shall be"],
    ["בוֹ","in him"],
    ["מַעְיַן","a well of"],
    ["מַיִם","water"],
    ["חַיִּים","living"],
    ["נוֹבֵעַ","springing up"],
    ["לְחַיֵּי","unto life of"],
    ["עוֹלָם׃","everlasting"]
  ]},
  { num: "כד", words: [
    ["וְעַתָּה","and now"],
    ["הִנֵּה","behold"],
    ["זֶה","this is"],
    ["רְצוֹן","the will of"],
    ["יְהוָה","the Lord"],
    ["אֱלֹהֵיכֶם","your God"],
    ["עַל","concerning"],
    ["קְדוֹשָׁיו","his saints"],
    ["כִּי","that"],
    ["יֵאָסְפוּ","they should assemble"],
    ["יַחְדָּו","together"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["לֹא","not"],
    ["בְחִפָּזוֹן","in haste"],
    ["פֶּן","lest"],
    ["תִּהְיֶה","there should be"],
    ["מְבוּכָה","confusion"],
    ["הַמְּבִיאָה","which brings"],
    ["מַגֵּפָה׃","pestilence"]
  ]},
  { num: "כה", words: [
    ["הִנֵּה","behold"],
    ["אֶרֶץ","the land of"],
    ["צִיּוֹן","Zion"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מַחֲזִיקָהּ","hold it"],
    ["בְּיָדִי׃","in my hand"]
  ]},
  { num: "כו", words: [
    ["אֲבָל","nevertheless"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["נוֹתֵן","give"],
    ["לְקֵיסָר","unto Caesar"],
    ["אֲשֶׁר","things which"],
    ["לְקֵיסָר׃","are Caesar's"]
  ]},
  { num: "כז", words: [
    ["לָכֵן","wherefore"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["חָפֵץ","will"],
    ["כִּי","that"],
    ["תִּקְנוּ","you should purchase"],
    ["הָאֲרָצוֹת","the lands"],
    ["לְמַעַן","that"],
    ["יִהְיֶה","may be"],
    ["לָכֶם","unto you"],
    ["יִתְרוֹן","an advantage"],
    ["עַל","over"],
    ["הָעוֹלָם","the world"],
    ["לְמַעַן","that"],
    ["תִּהְיֶה","you may have"],
    ["לָכֶם","unto you"],
    ["גְּאֻלָּה","claim"],
    ["עַל","on"],
    ["הָעוֹלָם","the world"],
    ["לְבַל","that not"],
    ["יִתְעוֹרְרוּ","they be stirred up"],
    ["לָכַעַס׃","unto anger"]
  ]},
  { num: "כח", words: [
    ["כִּי","for"],
    ["הַשָּׂטָן","Satan"],
    ["נוֹתֵן","puts it"],
    ["בְּלִבָּם","in their hearts"],
    ["לָכַעַס","to anger"],
    ["עֲלֵיכֶם","against you"],
    ["וְלִשְׁפֹּךְ","and to the shedding of"],
    ["דָּם׃","blood"]
  ]},
  { num: "כט", words: [["לָכֵן","wherefore"], ["לֹא","not"], ["אֶרֶץ","the land of"], ["צִיּוֹן","Zion"], ["כִּי־אִם","but"], ["בְּמִקְנָה","by purchase"], ["אוֹ","or"], ["בְּדָם","by blood"], ["וְאִם","and if"], ["לֹא","not"], ["אֵין","there is no"], ["נַחֲלָה","inheritance"], ["לָכֶם׃","for you"]]},
  { num: "ל", words: [
    ["וְאִם","and if"],
    ["בְּמִקְנָה","by purchase"],
    ["הִנֵּה","behold"],
    ["אַשְׁרֵיכֶם׃","you are blessed"]
  ]},
  { num: "לא", words: [
    ["וְאִם","and if"],
    ["בְּדָם","by blood"],
    ["בַּאֲשֶׁר","since"],
    ["אֲסוּרִים","forbidden are"],
    ["אַתֶּם","you"],
    ["לִשְׁפֹּךְ","to shed"],
    ["דָּם","blood"],
    ["הִנֵּה","behold"],
    ["אֹיְבֵיכֶם","your enemies"],
    ["עֲלֵיכֶם","are upon you"],
    ["וְתֻכּוּ","and you shall be scourged"],
    ["מֵעִיר","from city"],
    ["לְעִיר","to city"],
    ["וּמִבֵּית","and from house of"],
    ["כְּנֶסֶת","assembly synagogue"],
    ["לְבֵית","to house of"],
    ["כְּנֶסֶת","assembly"],
    ["וְרַק","and only"],
    ["מְעַטִּים","few"],
    ["יַעַמְדוּ","shall stand"],
    ["לְקַבֵּל","to receive"],
    ["נַחֲלָה׃","an inheritance"]
  ]},
  { num: "לב", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["כּוֹעֵס","am angry"],
    ["עַל","with"],
    ["הָרְשָׁעִים","the wicked"],
    ["אֲנִי","I"],
    ["מוֹנֵעַ","am withholding"],
    ["רוּחִי","my Spirit"],
    ["מִיּוֹשְׁבֵי","from the inhabitants of"],
    ["הָאָרֶץ׃","the earth"]
  ]},
  { num: "לג", words: [
    ["נִשְׁבַּעְתִּי","I have sworn"],
    ["בְאַפִּי","in my anger"],
    ["וְגָזַרְתִּי","and decreed"],
    ["מִלְחָמוֹת","wars"],
    ["עַל","upon"],
    ["פְּנֵי","the face of"],
    ["הָאָרֶץ","the earth"],
    ["וְהָרְשָׁעִים","and the wicked"],
    ["יַהַרְגוּ","shall slay"],
    ["הָרְשָׁעִים","the wicked"],
    ["וּפַחַד","and fear"],
    ["יָבוֹא","shall come"],
    ["עַל","upon"],
    ["כׇּל","every"],
    ["אָדָם׃","man"]
  ]},
  { num: "לד", words: [
    ["וְגַם","and also"],
    ["הַקְּדוֹשִׁים","the saints"],
    ["כִּמְעַט","hardly"],
    ["יִמָּלְטוּ","shall escape"],
    ["אַף","nevertheless"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["עִמָּם","am with them"],
    ["וְאֵרֵד","and I will come down"],
    ["מִן","from"],
    ["הַשָּׁמַיִם","heaven"],
    ["מִלִּפְנֵי","from the presence of"],
    ["אָבִי","my Father"],
    ["וְאֹכַל","and I will consume"],
    ["הָרְשָׁעִים","the wicked"],
    ["בְּאֵשׁ","with fire"],
    ["אֲשֶׁר","which"],
    ["לֹא","not"],
    ["תִכְבֶּה׃","shall be quenched"]
  ]},
  { num: "לה", words: [["וְהִנֵּה","and behold"], ["זֶה","this is"], ["עוֹד","yet"], ["לֹא","not"], ["כִּי־אִם","but"], ["בְּעוֹד","by and"], ["מְהֵרָה׃","by"]]},
  { num: "לו", words: [
    ["לָכֵן","wherefore"],
    ["בַּאֲשֶׁר","seeing that"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["גָּזַרְתִּי","have decreed"],
    ["כׇּל","all"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["עַל","upon"],
    ["פְּנֵי","the face of"],
    ["הָאָרֶץ","the earth"],
    ["אֲנִי","I"],
    ["חָפֵץ","will"],
    ["כִּי","that"],
    ["יֵאָסְפוּ","should be assembled"],
    ["קְדוֹשַׁי","my saints"],
    ["עַל","upon"],
    ["אֶרֶץ","the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "לז", words: [
    ["וְיִקַּח","and shall take"],
    ["כׇּל","every"],
    ["אִישׁ","man"],
    ["צֶדֶק","righteousness"],
    ["בְּיָדָיו","in his hands"],
    ["וֶאֱמוּנָה","and faithfulness"],
    ["עַל","upon"],
    ["מׇתְנָיו","his loins"],
    ["וְיִשָּׂא","and shall lift"],
    ["קוֹל","a voice of"],
    ["אַזְהָרָה","warning"],
    ["לְיוֹשְׁבֵי","unto the inhabitants of"],
    ["הָאָרֶץ","the earth"],
    ["וְיַכְרִיז","and shall declare"],
    ["בְּאֹמֶר","by word"],
    ["וּבִמְנוּסָה","and by flight"],
    ["כִּי","that"],
    ["שְׁמָמָה","desolation"],
    ["תָבוֹא","shall come"],
    ["עַל","upon"],
    ["הָרְשָׁעִים׃","the wicked"]
  ]},
  { num: "לח", words: [
    ["לָכֵן","wherefore"],
    ["יַעַרְכוּ","let arrange"],
    ["תַלְמִידַי","my disciples"],
    ["בְּקִירְטְלַנְד","in Kirtland"],
    ["אֶת־עִנְיְנֵיהֶם","their concerns"],
    ["הַגַּשְׁמִיִּים","temporal"],
    ["הַיּוֹשְׁבִים","who dwell"],
    ["בַּחַוָּה","upon farm"],
    ["הַזֹּאת׃","this"]
  ]},
  { num: "לט", words: [
    ["עַבְדִּי","my servant"],
    ["טִיטוּס","Titus"],
    ["בִּילִינְגְס","Billings"],
    ["הַנִּצָּב","who is responsible"],
    ["עָלֶיהָ","over it"],
    ["יִמְכֹּר","let dispose of"],
    ["הָאָרֶץ","the land"],
    ["לְמַעַן","so that"],
    ["יִהְיֶה","he may be"],
    ["נָכוֹן","prepared"],
    ["בָּאָבִיב","in the spring"],
    ["הַבָּא","the coming"],
    ["לָצֵאת","to go"],
    ["לְדַרְכּוֹ","on his journey"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["עִם","with"],
    ["הַיּוֹשְׁבִים","those who dwell"],
    ["עָלֶיהָ","upon it"],
    ["חוּץ","except"],
    ["מֵאֵלֶּה","from those"],
    ["אֲשֶׁר","whom"],
    ["אַשְׁאִיר","I shall reserve"],
    ["לִי","unto myself"],
    ["אֲשֶׁר","that"],
    ["לֹא","not"],
    ["יֵלְכוּ","shall go"],
    ["עַד","until"],
    ["אֲצַוֵּם׃","I shall command them"]
  ]},
  { num: "מ", words: [
    ["וְכׇל","and all"],
    ["הַכֶּסֶף","the money"],
    ["אֲשֶׁר","which"],
    ["יוּכַל","can"],
    ["לְהִשָּׁאֵר","be spared"],
    ["אַחַת","it matters"],
    ["הִיא","not"],
    ["לִי","unto me"],
    ["אִם","whether"],
    ["מְעַט","little"],
    ["אוֹ","or"],
    ["רַב","much"],
    ["יִשָּׁלַח","let be sent"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["לַאֲשֶׁר","unto whom"],
    ["מִנִּיתִי","I have appointed"],
    ["לְקַבֵּל׃","to receive"]
  ]},
  { num: "מא", words: [["הִנֵּה","behold"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["אֶתֵּן","will give"], ["לְעַבְדִּי","unto my servant"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["כֹּחַ","power"], ["לְהַבְחִין","to discern"], ["בָּרוּחַ","by the Spirit"], ["אֵת־הָעֹלִים","those going up"], ["לְאֶרֶץ","unto the land of"], ["צִיּוֹן","Zion"], ["וְאֵת","and"], ["תַּלְמִידַי","my disciples"], ["הַנִּשְׁאָרִים׃","who shall tarry"]]},
  { num: "מב", words: [
    ["עַבְדִּי","my servant"],
    ["נְיוּאֶל","Newel"],
    ["ק.","K."],
    ["וִיטְנִי","Whitney"],
    ["יִשְׁמֹר","let retain"],
    ["חֲנוּתוֹ","his store"],
    ["אוֹ","or"],
    ["בְּמִלִּים","in words"],
    ["אֲחֵרוֹת","other"],
    ["הַחֲנוּת","the store"],
    ["עוֹד","yet"],
    ["לַמִּצְעָר׃","for a little season"]
  ]},
  { num: "מג", words: [
    ["אֲבָל","nevertheless"],
    ["יִתֵּן","let him impart"],
    ["כׇּל","all"],
    ["הַכֶּסֶף","the money"],
    ["אֲשֶׁר","which"],
    ["יוּכַל","he can"],
    ["לָתֵת","impart"],
    ["לְהִשָּׁלֵחַ","to be sent"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "מד", words: [
    ["הִנֵּה","behold"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["בְּיָדָיו","are in his hands"],
    ["יַעֲשֶׂה","let him do"],
    ["לְפִי","according to"],
    ["חׇכְמָה׃","wisdom"]
  ]},
  { num: "מה", words: [
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["יְמֻנֶּה","let him be ordained"],
    ["לְסֹכֵן","as an agent"],
    ["לַתַּלְמִידִים","unto the disciples"],
    ["הַנִּשְׁאָרִים","that shall tarry"],
    ["וְיִסָּמֵךְ","and let him be ordained"],
    ["לַכֹּחַ","unto power"],
    ["הַזֶּה׃","this"]
  ]},
  { num: "מו", words: [["וְעַתָּה","and now"], ["בִּמְהֵרָה","speedily"], ["יְבַקֵּר","let him visit"], ["הַכְּנֵסִיּוֹת","the churches"], ["וִיבָאֵר","and expound"], ["לָהֶם","unto them"], ["הַדְּבָרִים","the things"], ["הָאֵלֶּה","these"], ["עִם","with"], ["עַבְדִּי","my servant"], ["אוֹלִיבֶר","Oliver"], ["קַאוּדְרִי","Cowdery"], ["הִנֵּה","behold"], ["זֶה","this is"], ["רְצוֹנִי","my will"], ["לְהַשִּׂיג","to obtain"], ["כְּסָפִים","moneys"], ["כַּאֲשֶׁר","even as"], ["הוֹרֵיתִי׃","I have directed"]]},
  { num: "מז", words: [
    ["הַנֶּאֱמָן","he that is faithful"],
    ["וְהָעוֹמֵד","and endures"],
    ["יִגְבַּר","shall overcome"],
    ["עַל","upon"],
    ["הָעוֹלָם׃","the world"]
  ]},
  { num: "מח", words: [
    ["הַשּׁוֹלֵחַ","he that sends up"],
    ["אוֹצָרוֹת","treasures"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["יְקַבֵּל","shall receive"],
    ["נַחֲלָה","an inheritance"],
    ["בָּעוֹלָם","in the world"],
    ["הַזֶּה","this"],
    ["וּמַעֲשָׂיו","and his works"],
    ["יֵלְכוּ","shall follow"],
    ["אַחֲרָיו","after him"],
    ["וְגַם","and also"],
    ["שָׂכָר","a reward"],
    ["בָּעוֹלָם","in the world"],
    ["הַבָּא׃","that is coming"]
  ]},
  { num: "מט", words: [
    ["אָכֵן","yea"],
    ["וְאַשְׁרֵי","and blessed are"],
    ["הַמֵּתִים","the dead"],
    ["הַמֵּתִים","that die"],
    ["בַּיהוָה","in the Lord"],
    ["מֵעַתָּה","from henceforth"],
    ["בְּבוֹא","when shall come"],
    ["יְהוָה","the Lord"],
    ["וְיַעַבְרוּ","and shall pass away"],
    ["דְּבָרִים","things"],
    ["יְשָׁנִים","old"],
    ["וְכׇל","and all"],
    ["הַדְּבָרִים","the things"],
    ["יִהְיוּ","shall become"],
    ["חֲדָשִׁים","new"],
    ["יָקוּמוּ","they shall rise"],
    ["מִן","from"],
    ["הַמֵּתִים","the dead"],
    ["וְלֹא","and not"],
    ["יָמוּתוּ","shall die"],
    ["אַחֲרֵי","after"],
    ["כֵן","that"],
    ["וִיקַבְּלוּ","and shall receive"],
    ["נַחֲלָה","an inheritance"],
    ["לִפְנֵי","before"],
    ["יְהוָה","the Lord"],
    ["בָּעִיר","in the city"],
    ["הַקְּדוֹשָׁה׃","holy"]
  ]},
  { num: "נ", words: [
    ["וְהַחַי","and he that lives"],
    ["בְּבוֹא","when shall come"],
    ["יְהוָה","the Lord"],
    ["וְשָׁמַר","and has kept"],
    ["הָאֱמוּנָה","the faith"],
    ["אַשְׁרָיו","blessed is he"],
    ["אֲבָל","nevertheless"],
    ["נִגְזַר","it is appointed"],
    ["עָלָיו","unto him"],
    ["לָמוּת","to die"],
    ["בִּימֵי","at the age of"],
    ["אָדָם׃","man"]
  ]},
  { num: "נא", words: [["לָכֵן","wherefore"], ["יְלָדִים","children"], ["יִגְדְּלוּ","shall grow up"], ["עַד","until"], ["אֲשֶׁר","that"], ["יִזְקְנוּ","they become old"], ["זְקֵנִים","old men"], ["יָמוּתוּ","shall die"], ["אַךְ","but"], ["לֹא","not"], ["יִישְׁנוּ","they shall sleep"], ["בֶעָפָר","in the dust"], ["כִּי־אִם","but"], ["יְשֻׁנּוּ","they shall be changed"], ["כְּהֶרֶף","in the twinkling of"], ["עָיִן׃","an eye"]]},
  { num: "נב", words: [
    ["לָכֵן","wherefore"],
    ["לָזֹאת","for this cause"],
    ["קָרְאוּ","preached"],
    ["הַשְּׁלוּחִים","the apostles"],
    ["לָעוֹלָם","unto the world"],
    ["תְּחִיַּת","the resurrection of"],
    ["הַמֵּתִים׃","the dead"]
  ]},
  { num: "נג", words: [
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["הֵם","are those"],
    ["אֲשֶׁר","which"],
    ["עֲלֵיכֶם","upon you"],
    ["לְצַפּוֹת","to look"],
    ["לָהֶם","for them"],
    ["וּבְדַבֵּר","and in speaking"],
    ["כְּדֶרֶךְ","after the manner of"],
    ["יְהוָה","the Lord"],
    ["הֵם","they are"],
    ["קְרוֹבִים","nigh"],
    ["עַתָּה","now"],
    ["וּבְעֵת","and in a time"],
    ["לָבוֹא","to come"],
    ["בְּיוֹם","in the day of"],
    ["בּוֹא","the coming of"],
    ["בֶן","the Son of"],
    ["הָאָדָם׃","Man"]
  ]},
  { num: "נד", words: [
    ["וְעַד","and until"],
    ["הַשָּׁעָה","the hour"],
    ["הַהִיא","that"],
    ["יִהְיוּ","there will be"],
    ["בְּתוּלוֹת","virgins"],
    ["כְּסִילוֹת","foolish"],
    ["בֵּין","among"],
    ["הַחֲכָמוֹת","the wise"],
    ["וּבַשָּׁעָה","and at the hour"],
    ["הַהִיא","that"],
    ["תָּבוֹא","shall come"],
    ["חֲלֻקָּה","a separation"],
    ["שְׁלֵמָה","entire"],
    ["בֵּין","between"],
    ["הַצַּדִּיקִים","the righteous"],
    ["וְהָרְשָׁעִים","and the wicked"],
    ["וּבַיּוֹם","and in the day"],
    ["הַהוּא","that"],
    ["אֶשְׁלַח","I will send"],
    ["מַלְאָכַי","my angels"],
    ["לַעֲקֹר","to pluck out"],
    ["הָרְשָׁעִים","the wicked"],
    ["וּלְהַשְׁלִיכָם","and to cast them"],
    ["בְּאֵשׁ","into fire"],
    ["אֲשֶׁר","which"],
    ["לֹא","not"],
    ["תִכְבֶּה׃","shall be quenched"]
  ]},
  { num: "נה", words: [["וְעַתָּה","and now"], ["הִנֵּה","behold"], ["אָמֵן","verily"], ["אֲנִי","I"], ["אֹמֵר","say"], ["לָכֶם","unto you"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["אֵינֶנִּי","am not"], ["רָצוּי","pleased"], ["בְּעַבְדִּי","with my servant"], ["סִידְנִי","Sidney"], ["רִיגְדּוֹן","Rigdon"], ["הִתְנַשֵּׂא","he exalted himself"], ["בְּלִבּוֹ","in his heart"], ["וְלֹא","and not"], ["קִבֵּל","received"], ["עֵצָה","counsel"], ["כִּי־אִם","but"], ["הֶעֱצִיב","grieved"], ["הָרוּחַ׃","the Spirit"]]},
  { num: "נו", words: [
    ["לָכֵן","wherefore"],
    ["כְּתָבוֹ","his writing"],
    ["אֵינֶנָּה","is not"],
    ["רְצוּיָה","acceptable"],
    ["לַיהוָה","unto the Lord"],
    ["וְיַעֲשֶׂה","and he shall make"],
    ["אַחֶרֶת","another"],
    ["וְאִם","and if"],
    ["לֹא","not"],
    ["יְקַבְּלָהּ","receive it"],
    ["יְהוָה","the Lord"],
    ["הִנֵּה","behold"],
    ["אֵינֶנּוּ","he is not"],
    ["עוֹמֵד","standing"],
    ["עוֹד","longer"],
    ["בַּמִּשְׂרָה","in the office"],
    ["אֲשֶׁר","to which"],
    ["מִנִּיתִיו׃","I have appointed him"]
  ]},
  { num: "נז", words: [
    ["וְעוֹד","and again"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["הַחֲפֵצִים","those who desire"],
    ["בְּלִבָּם","in their hearts"],
    ["בַּעֲנָוָה","in meekness"],
    ["לְהַזְהִיר","to warn"],
    ["חוֹטְאִים","sinners"],
    ["לִתְשׁוּבָה","to repentance"],
    ["יִסָּמְכוּ","let them be ordained"],
    ["לַכֹּחַ","unto power"],
    ["הַזֶּה׃","this"]
  ]},
  { num: "נח", words: [
    ["כִּי","for"],
    ["זֶה","this is"],
    ["יוֹם","a day of"],
    ["אַזְהָרָה","warning"],
    ["וְלֹא","and not"],
    ["יוֹם","a day of"],
    ["דְּבָרִים","words"],
    ["רַבִּים","many"],
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["לֹא","not"],
    ["יִתְלוֹצְצוּ","shall they mock"],
    ["בִּי","at me"],
    ["בְּאַחֲרִית","in the latter part of"],
    ["הַיָּמִים׃","the days"]
  ]},
  { num: "נט", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["מִמַּעַל","am from above"],
    ["וְכֹחִי","and my power"],
    ["מִתַּחַת","lies beneath"],
    ["אֲנִי","I am"],
    ["עַל","over"],
    ["הַכֹּל","all"],
    ["וּבַכֹּל","and in all"],
    ["וּבְתוֹךְ","and through"],
    ["הַכֹּל","all"],
    ["וְחוֹקֵר","and search"],
    ["כׇּל","all"],
    ["דָּבָר","thing"],
    ["וְהַיּוֹם","and the day"],
    ["יָבוֹא","comes"],
    ["אֲשֶׁר","in which"],
    ["כׇּל","all"],
    ["הַדְּבָרִים","things"],
    ["יִכָּנְעוּ","shall be subject"],
    ["לִי׃","unto me"]
  ]},
  { num: "ס", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I am"],
    ["אָלֶף","Alpha"],
    ["וְתָו","and Omega"],
    ["יֵשׁוּעַ","Jesus"],
    ["הַמָּשִׁיחַ׃","the Christ"]
  ]},
  { num: "סא", words: [
    ["לָכֵן","wherefore"],
    ["יִשָּׁמְרוּ","let beware"],
    ["כׇל","all"],
    ["בְּנֵי","sons of"],
    ["אָדָם","man"],
    ["אֵיךְ","how"],
    ["יִקְחוּ","they take"],
    ["שְׁמִי","my name"],
    ["בִּשְׂפָתֵיהֶם׃","in their lips"]
  ]},
  { num: "סב", words: [
    ["כִּי","for"],
    ["הִנֵּה","behold"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["רַבִּים","many"],
    ["יֵשׁ","there are"],
    ["תַּחַת","under"],
    ["הָאַשְׁמָה","the condemnation"],
    ["הַזֹּאת","this"],
    ["הַלֹּקְחִים","who use"],
    ["בְּשֵׁם","the name of"],
    ["יְהוָה","the Lord"],
    ["וְנֹשְׂאִים","and take"],
    ["אוֹתוֹ","it"],
    ["לַשָּׁוְא","in vain"],
    ["בְּאֵין","without"],
    ["שִׁלְטוֹן׃","authority"]
  ]},
  { num: "סג", words: [["לָכֵן","wherefore"], ["תָּשׁוּב","let repent"], ["הַכְּנֵסִיָּה","the church"], ["מֵחֲטָאֶיהָ","of her sins"], ["וַאֲנִי","and I"], ["יְהוָה","the Lord"], ["אֲקַבְּלֵם","will own them"], ["וְאִם","and if"], ["לֹא","not"], ["יִכָּרְתוּ׃","they shall be cut off"]]},
  { num: "סד", words: [
    ["זִכְרוּ","remember"],
    ["כִּי","that"],
    ["הַבָּא","that which comes"],
    ["מִמַּעַל","from above"],
    ["קָדוֹשׁ","sacred"],
    ["הוּא","is"],
    ["וְנָכוֹן","and must"],
    ["לְהֵאָמֵר","be spoken"],
    ["בְּיִרְאָה","with care"],
    ["וּבְאֹנֶס","and by constraint of"],
    ["הָרוּחַ","the Spirit"],
    ["וּבָזֶה","and in this"],
    ["אֵין","there is no"],
    ["אַשְׁמָה","condemnation"],
    ["וּמְקַבְּלִים","and you receive"],
    ["אַתֶּם","you"],
    ["הָרוּחַ","the Spirit"],
    ["בִּתְפִלָּה","through prayer"],
    ["לָכֵן","wherefore"],
    ["בִּלְעָדֶיהָ","without this"],
    ["נִשְׁאֶרֶת","remains"],
    ["אַשְׁמָה׃","condemnation"]
  ]},
  { num: "סה", words: [["עֲבָדַי","my servants"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["וְסִידְנִי","and Sidney"], ["רִיגְדּוֹן","Rigdon"], ["יְבַקְּשׁוּ","let them seek"], ["לָהֶם","for themselves"], ["בַּיִת","a home"], ["כַּאֲשֶׁר","as"], ["יִלָּמְדוּ","they are taught"], ["בִּתְפִלָּה","through prayer"], ["בְּיַד","by the means of"], ["הָרוּחַ׃","the Spirit"]]},
  { num: "סו", words: [["הַדְּבָרִים","the things"], ["הָאֵלֶּה","these"], ["נִשְׁאָרִים","remain"], ["לִגְבֹּר","to overcome"], ["עֲלֵיהֶם","through them"], ["בְּאֹרֶךְ־אַפַּיִם","through patience"], ["לְמַעַן","that"], ["יְקַבְּלוּ","may receive"], ["כָאֵלֶּה","such"], ["מִשְׁקַל","a weight of"], ["כָּבוֹד","glory"], ["יוֹתֵר","more"], ["גָּדוֹל","exceeding"], ["עֲדֵי־עַד","and eternal"], ["וְאִם","and if"], ["לֹא","not"], ["אַשְׁמָה","condemnation"], ["גְדוֹלָה","greater"], ["יוֹתֵר","more"], ["אָמֵן׃","amen"]]}
];
renderVerseSet(dc63_ch1Verses, 'dc63-ch1-verses');



var dc64_ch1Verses = [
  { num: "א", words: [["הִנֵּה","behold"], ["כֹּה","thus"], ["אָמַר","says"], ["יְהוָה","the Lord"], ["אֱלֹהֵיכֶם","your God"], ["אֲלֵיכֶם","unto you"], ["זִקְנֵי","O elders of"], ["כְּנֵסִיָּתִי","my church"], ["הַקְשִׁיבוּ","hearken"], ["וְשִׁמְעוּ","and hear"], ["וְקַבְּלוּ","and receive"], ["רְצוֹנִי","my will"], ["עֲלֵיכֶם׃","concerning you"]]},
  { num: "ב", words: [
    ["כִּי","for"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["אֲנִי","I"],
    ["חָפֵץ","will"],
    ["כִּי","that"],
    ["תִּגְבְּרוּ","you should overcome"],
    ["עַל","upon"],
    ["הָעוֹלָם","the world"],
    ["לָכֵן","wherefore"],
    ["אֲרַחֵם","I will have compassion"],
    ["עֲלֵיכֶם׃","upon you"]
  ]},
  { num: "ג", words: [
    ["יֵשׁ","there are"],
    ["בֵּינֵיכֶם","among you"],
    ["אֲשֶׁר","those who"],
    ["חָטְאוּ","have sinned"],
    ["אֲבָל","but"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לַפַּעַם","for once"],
    ["הַזֹּאת","this"],
    ["לִכְבוֹדִי","for my own glory"],
    ["וְלִישׁוּעַת","and for the salvation of"],
    ["נְפָשׁוֹת","souls"],
    ["סָלַחְתִּי","I have forgiven"],
    ["לָכֶם","you"],
    ["חַטֹּאתֵיכֶם׃","your sins"]
  ]},
  { num: "ד", words: [
    ["אֲרַחֵם","I will be merciful"],
    ["עֲלֵיכֶם","unto you"],
    ["כִּי","for"],
    ["נָתַתִּי","I have given"],
    ["לָכֶם","unto you"],
    ["הַמַּלְכוּת׃","the kingdom"]
  ]},
  { num: "ה", words: [["וּמַפְתְּחוֹת","and the keys of"], ["סוֹדוֹת","the mysteries of"], ["הַמַּלְכוּת","the kingdom"], ["לֹא","not"], ["יִלָּקְחוּ","shall be taken"], ["מֵעַבְדִּי","from my servant"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["בַּדְּרָכִים","through the means"], ["אֲשֶׁר","which"], ["מִנִּיתִי","I have appointed"], ["כׇּל","all"], ["יְמֵי","the days of"], ["חַיָּיו","his life"], ["אִם","inasmuch as"], ["יִשְׁמֹר","he obeys"], ["חֻקּוֹתָי׃","my ordinances"]]},
  { num: "ו", words: [
    ["יֵשׁ","there are those"],
    ["אֲשֶׁר","who"],
    ["בִּקְּשׁוּ","have sought"],
    ["עִלָּה","occasion"],
    ["עָלָיו","against him"],
    ["בְּלֹא","without"],
    ["סִבָּה׃","cause"]
  ]},
  { num: "ז", words: [
    ["אֲבָל","nevertheless"],
    ["הוּא","he"],
    ["חָטָא","has sinned"],
    ["אַךְ","but"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["סוֹלֵחַ","forgive"],
    ["חֲטָאִים","sins"],
    ["לַמִּתְוַדִּים","unto those who confess"],
    ["חֲטָאֵיהֶם","their sins"],
    ["לְפָנַי","before me"],
    ["וּמְבַקְּשִׁים","and ask"],
    ["סְלִיחָה","forgiveness"],
    ["אֲשֶׁר","who"],
    ["לֹא","not"],
    ["חָטְאוּ","have sinned"],
    ["לַמָּוֶת׃","unto death"]
  ]},
  { num: "ח", words: [
    ["תַּלְמִידַי","my disciples"],
    ["בְּיָמִים","in days"],
    ["קַדְמוֹנִים","of old"],
    ["בִּקְּשׁוּ","sought"],
    ["עִלָּה","occasion"],
    ["זֶה","one"],
    ["עַל","against"],
    ["זֶה","another"],
    ["וְלֹא","and not"],
    ["סָלְחוּ","forgave"],
    ["זֶה","one"],
    ["לָזֶה","another"],
    ["בְּלִבָּם","in their hearts"],
    ["וּבַעֲבוּר","and for"],
    ["הָרָעָה","evil"],
    ["הַזֹּאת","this"],
    ["הֻכּוּ","they were afflicted"],
    ["וְנִוַּסְּרוּ","and chastened"],
    ["קָשֶׁה׃","sorely"]
  ]},
  { num: "ט", words: [
    ["לָכֵן","wherefore"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["עֲלֵיכֶם","you ought"],
    ["לִסְלֹחַ","to forgive"],
    ["זֶה","one"],
    ["לָזֶה","another"],
    ["כִּי","for"],
    ["אֲשֶׁר","he that"],
    ["אֵינֶנּוּ","not"],
    ["סוֹלֵחַ","forgives"],
    ["לְאָחִיו","his brother"],
    ["פְּשָׁעָיו","his trespasses"],
    ["עוֹמֵד","stands"],
    ["מֻרְשָׁע","condemned"],
    ["לִפְנֵי","before"],
    ["יְהוָה","the Lord"],
    ["כִּי","for"],
    ["נִשְׁאָר","remains"],
    ["בּוֹ","in him"],
    ["הַחֵטְא","the sin"],
    ["הַגָּדוֹל","great"],
    ["יוֹתֵר׃","more"]
  ]},
  { num: "י", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אֶסְלַח","will forgive"],
    ["לַאֲשֶׁר","whom"],
    ["אֶסְלָח","I will forgive"],
    ["אַךְ","but"],
    ["מִכֶּם","of you"],
    ["נִדְרָשׁ","it is required"],
    ["לִסְלֹחַ","to forgive"],
    ["לְכׇל","all"],
    ["אָדָם׃","men"]
  ]},
  { num: "יא", words: [
    ["וַאֲמַרְתֶּם","and you ought to say"],
    ["בִּלְבַבְכֶם","in your hearts"],
    ["יִשְׁפֹּט","let judge"],
    ["אֱלֹהִים","God"],
    ["בֵּינִי","between me"],
    ["וּבֵינֶךָ","and you"],
    ["וְיִגְמׇלְךָ","and reward you"],
    ["כְּמַעֲשֶׂיךָ׃","according to your deeds"]
  ]},
  { num: "יב", words: [["וְאֵת","and"], ["אֲשֶׁר","him that"], ["אֵינֶנּוּ","not"], ["שָׁב","repents"], ["מֵחֲטָאָיו","of his sins"], ["וְאֵינֶנּוּ","and not"], ["מִתְוַדֶּה","confesses"], ["עֲלֵיהֶם","them"], ["תָּבִיאוּ","you shall bring"], ["לִפְנֵי","before"], ["הַכְּנֵסִיָּה","the church"], ["וַעֲשִׂיתֶם","and do"], ["לוֹ","unto him"], ["כַּאֲשֶׁר","as"], ["אוֹמֵר","says"], ["הַכָּתוּב","the scripture"], ["אוֹ","either"], ["בְמִצְוָה","by commandment"], ["אוֹ","or"], ["בְחָזוֹן׃","by revelation"]]},
  { num: "יג", words: [["וְזֹאת","and this"], ["תַּעֲשׂוּ","you shall do"], ["לְמַעַן","that"], ["יִכָּבֵד","may be glorified"], ["אֱלֹהִים","God"], ["לֹא","not"], ["בַּאֲשֶׁר","because"], ["אֵינְכֶם","you not"], ["סֹלְחִים","forgive"], ["בְּאֵין","without"], ["רַחֲמִים","compassion"], ["כִּי־אִם","but"], ["לְמַעַן","that"], ["תִּצָּדְקוּ","you may be justified"], ["לְפִי","in the eyes of"], ["הַתּוֹרָה","the law"], ["לְבַל","that not"], ["תַּכְעִיסוּ","you offend"], ["אֶת־נוֹתֵן","the Giver of"], ["הַתּוֹרָה׃","the law"]]},
  { num: "יד", words: [
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָזֹאת","for this cause"],
    ["תַּעֲשׂוּ","you shall do"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה׃","these"]
  ]},
  { num: "טו", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["כָּעַסְתִּי","was angry"],
    ["עַל","with"],
    ["אֲשֶׁר","him who"],
    ["הָיָה","was"],
    ["עַבְדִּי","my servant"],
    ["עֶזְרָא","Ezra"],
    ["בּוּת","Booth"],
    ["וְגַם","and also"],
    ["עַבְדִּי","my servant"],
    ["יִצְחָק","Isaac"],
    ["מוֹרְלִי","Morley"],
    ["כִּי","for"],
    ["לֹא","not"],
    ["שָׁמְרוּ","they kept"],
    ["הַתּוֹרָה","the law"],
    ["וְלֹא","neither"],
    ["הַמִּצְוָה׃","the commandment"]
  ]},
  { num: "טז", words: [
    ["בִּקְּשׁוּ","they sought"],
    ["רָעָה","evil"],
    ["בְּלִבָּם","in their hearts"],
    ["וַאֲנִי","and I"],
    ["יְהוָה","the Lord"],
    ["מָנַעְתִּי","withheld"],
    ["רוּחִי","my Spirit"],
    ["הִרְשִׁיעוּ","they condemned"],
    ["לָרַע","as evil"],
    ["אֵת־אֲשֶׁר","that which"],
    ["אֵין","there is no"],
    ["בּוֹ","in it"],
    ["רַע","evil"],
    ["אֲבָל","nevertheless"],
    ["סָלַחְתִּי","I have forgiven"],
    ["לְעַבְדִּי","my servant"],
    ["יִצְחָק","Isaac"],
    ["מוֹרְלִי׃","Morley"]
  ]},
  { num: "יז", words: [
    ["וְגַם","and also"],
    ["עַבְדִּי","my servant"],
    ["אֶדְוָרְד","Edward"],
    ["פַּרְטְרִידְג","Partridge"],
    ["הִנֵּה","behold"],
    ["הוּא","he"],
    ["חָטָא","has sinned"],
    ["וְהַשָּׂטָן","and Satan"],
    ["מְבַקֵּשׁ","seeks"],
    ["לְהַשְׁחִית","to destroy"],
    ["נַפְשׁוֹ","his soul"],
    ["אַךְ","but"],
    ["כַּאֲשֶׁר","when"],
    ["יֻוָּדְעוּ","are made known"],
    ["לָהֶם","unto them"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["וְיָשׁוּבוּ","and they repent"],
    ["מֵהָרַע","of the evil"],
    ["יִסָּלַח","it shall be forgiven"],
    ["לָהֶם׃","unto them"]
  ]},
  { num: "יח", words: [
    ["וְעַתָּה","and now"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["רָצוּי","it is expedient"],
    ["לִי","in me"],
    ["כִּי","that"],
    ["עַבְדִּי","my servant"],
    ["סִידְנִי","Sidney"],
    ["גִילְבֶּרְט","Gilbert"],
    ["אַחֲרֵי","after"],
    ["שָׁבוּעוֹת","weeks"],
    ["אֲחָדִים","a few"],
    ["יָשׁוּב","shall return"],
    ["לִמְלַאכְתּוֹ","upon his business"],
    ["וּלְמִשְׁלַחְתּוֹ","and to his agency"],
    ["בְּאֶרֶץ","in the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "יט", words: [
    ["וַאֲשֶׁר","and that which"],
    ["רָאָה","he has seen"],
    ["וְשָׁמַע","and heard"],
    ["יֻוָּדַע","may be made known"],
    ["לְתַלְמִידַי","unto my disciples"],
    ["לְבַל","that not"],
    ["יֹאבֵדוּ","they perish"],
    ["וְלָזֹאת","and for this cause"],
    ["דִּבַּרְתִּי","have I spoken"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה׃","these"]
  ]},
  { num: "כ", words: [
    ["וְעוֹד","and again"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["לְמַעַן","that"],
    ["לֹא","not"],
    ["יְנֻסֶּה","may be tempted"],
    ["עַבְדִּי","my servant"],
    ["יִצְחָק","Isaac"],
    ["מוֹרְלִי","Morley"],
    ["יוֹתֵר","more"],
    ["מִנְּשֹׂא","than he is able to bear"],
    ["וְיָעֵץ","and counsel"],
    ["בְּעַוְלָה","wrongfully"],
    ["לְרָעַתְכֶם","to your hurt"],
    ["צִוִּיתִי","I gave commandment"],
    ["כִּי","that"],
    ["תִּמָּכֵר","should be sold"],
    ["חַוָּתוֹ׃","his farm"]
  ]},
  { num: "כא", words: [
    ["אֵינֶנִּי","I am not"],
    ["חָפֵץ","willing"],
    ["כִּי","that"],
    ["עַבְדִּי","my servant"],
    ["פְרֶדֶרִיק","Frederick"],
    ["ג׳.","G."],
    ["וִילְיָמְס","Williams"],
    ["יִמְכֹּר","should sell"],
    ["חַוָּתוֹ","his farm"],
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["חָפֵץ","will"],
    ["לְהַחֲזִיק","to retain"],
    ["מָעוֹז","a stronghold"],
    ["חָזָק","strong"],
    ["בְּאֶרֶץ","in the land of Kirtland"],
    ["חָמֵשׁ","for the space of five"],
    ["שָׁנִים","years"],
    ["בָּהֶן","in which"],
    ["לֹא","not"],
    ["אַהֲפֹךְ","will I overthrow"],
    ["הָרְשָׁעִים","the wicked"],
    ["לְמַעַן","that thereby"],
    ["אוֹשִׁיעַ","I may save"],
    ["אֲחָדִים׃","some"]
  ]},
  { num: "כב", words: [
    ["וְאַחֲרֵי","and after"],
    ["הַיּוֹם","the day"],
    ["הַהוּא","that"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["לֹא","not"],
    ["אַרְשִׁיעַ","will hold guilty"],
    ["כׇּל","any"],
    ["הָעֹלֶה","that goes up"],
    ["בְּלֵב","with heart"],
    ["פָּתוּחַ","open"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["דּוֹרֵשׁ","require"],
    ["לִבּוֹת","the hearts of"],
    ["בְּנֵי","the children of"],
    ["אָדָם׃","men"]
  ]},
  { num: "כג", words: [
    ["הִנֵּה","behold"],
    ["עַתָּה","now"],
    ["נִקְרָא","it is called"],
    ["הַיּוֹם","today"],
    ["עַד","until"],
    ["בּוֹא","the coming of"],
    ["בֶן","the Son of"],
    ["הָאָדָם","Man"],
    ["וְאָמֵן","and verily"],
    ["הוּא","it is"],
    ["יוֹם","a day of"],
    ["קָרְבָּן","sacrifice"],
    ["וְיוֹם","and a day"],
    ["לְמַעְשַׂר","for the tithing of"],
    ["עַמִּי","my people"],
    ["כִּי","for"],
    ["הַמְעֻשָּׂר","he that is tithed"],
    ["לֹא","not"],
    ["יִשָּׂרֵף","shall be burned"],
    ["בְּבוֹאוֹ׃","at his coming"]
  ]},
  { num: "כד", words: [
    ["כִּי","for"],
    ["אַחֲרֵי","after"],
    ["הַיּוֹם","today"],
    ["תָּבוֹא","comes"],
    ["הַשְּׂרֵפָה","the burning"],
    ["זֶה","this is"],
    ["לְדַבֵּר","speaking"],
    ["כְּדֶרֶךְ","after the manner of"],
    ["יְהוָה","the Lord"],
    ["כִּי","for"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["מָחָר","tomorrow"],
    ["כׇּל","all"],
    ["הַגֵּאִים","the proud"],
    ["וְעֹשֵׂי","and those doing"],
    ["רֶשַׁע","wickedness"],
    ["יִהְיוּ","shall be"],
    ["כַּקַּשׁ","as stubble"],
    ["וּשְׂרַפְתִּים","and I will burn them up"],
    ["כִּי","for"],
    ["אֲנִי","I am"],
    ["יְהוָה","the Lord of"],
    ["צְבָאוֹת","Hosts"],
    ["וְלֹא","and not"],
    ["אֶחְמֹל","will I spare"],
    ["עַל","upon"],
    ["כׇּל","any"],
    ["הַנִּשְׁאָרִים","that remain"],
    ["בְּבָבֶל׃","in Babylon"]
  ]},
  { num: "כה", words: [
    ["לָכֵן","wherefore"],
    ["אִם","if"],
    ["תַּאֲמִינוּ","you believe"],
    ["לִי","me"],
    ["תַּעַבְדוּ","you will labor"],
    ["בְּעוֹד","while"],
    ["נִקְרָא","it is called"],
    ["הַיּוֹם׃","today"]
  ]},
  { num: "כו", words: [["וְאֵין","and it is not"], ["רָאוּי","meet"], ["כִּי","that"], ["עֲבָדַי","my servants"], ["נְיוּאֶל","Newel"], ["ק.","K."], ["וִיטְנִי","Whitney"], ["וְסִידְנִי","and Sidney"], ["גִילְבֶּרְט","Gilbert"], ["יִמְכְּרוּ","should sell"], ["חֲנוּתָם","their store"], ["וּרְכוּשָׁם","and their possessions"], ["פֹּה","here"], ["כִּי","for"], ["אֵין","is not"], ["זֹאת","this"], ["חׇכְמָה","wisdom"], ["עַד","until"], ["אֲשֶׁר","that"], ["שְׁאֵרִית","the residue of"], ["הַכְּנֵסִיָּה","the church"], ["הַנִּשְׁאֶרֶת","which remains"], ["בַּמָּקוֹם","in place"], ["הַזֶּה","this"], ["תַּעֲלֶה","shall go up"], ["לְאֶרֶץ","unto the land of"], ["צִיּוֹן׃","Zion"]]},
  { num: "כז", words: [
    ["הִנֵּה","behold"],
    ["נֶאֱמַר","it is said"],
    ["בְּתוֹרוֹתַי","in my laws"],
    ["אוֹ","or"],
    ["אָסוּר","forbidden"],
    ["לָבוֹא","to get"],
    ["בְּחוֹב","in debt"],
    ["לְאֹיְבֶיךָ׃","to your enemies"]
  ]},
  { num: "כח", words: [
    ["אַךְ","but"],
    ["הִנֵּה","behold"],
    ["לֹא","not"],
    ["נֶאֱמַר","it is said"],
    ["בְּכׇל","at any"],
    ["עֵת","time"],
    ["אֲשֶׁר","that"],
    ["לֹא","not"],
    ["יִקַּח","should take"],
    ["יְהוָה","the Lord"],
    ["כַּחֲפָצוֹ","when he please"],
    ["וִישַׁלֵּם","and pay"],
    ["כַּטּוֹב","as seems good"],
    ["בְּעֵינָיו׃","in his eyes"]
  ]},
  { num: "כט", words: [
    ["לָכֵן","wherefore"],
    ["בַּאֲשֶׁר","as"],
    ["אַתֶּם","you are"],
    ["סֹכְנִים","agents"],
    ["אַתֶּם","you are"],
    ["בְּמִשְׁלַחַת","on the errand of"],
    ["יְהוָה","the Lord"],
    ["וְכׇל","and whatever"],
    ["אֲשֶׁר","that"],
    ["תַּעֲשׂוּ","you do"],
    ["לְפִי","according to"],
    ["רְצוֹן","the will of"],
    ["יְהוָה","the Lord"],
    ["הוּא","it is"],
    ["מְלֶאכֶת","the business of"],
    ["יְהוָה׃","the Lord"]
  ]},
  { num: "ל", words: [
    ["וְהוּא","and he"],
    ["הִצִּיב","has set"],
    ["אֶתְכֶם","you"],
    ["לְכַלְכֵּל","to provide for"],
    ["קְדוֹשָׁיו","his saints"],
    ["בְּאַחֲרִית","in the latter part of"],
    ["הַיָּמִים","the days"],
    ["הָאֵלֶּה","these"],
    ["לְמַעַן","that"],
    ["יַשִּׂיגוּ","they may obtain"],
    ["נַחֲלָה","an inheritance"],
    ["בְּאֶרֶץ","in the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "לא", words: [
    ["וְהִנֵּה","and behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מַכְרִיז","declare"],
    ["לָכֶם","unto you"],
    ["וּדְבָרַי","and my words"],
    ["נֶאֱמָנִים","are sure"],
    ["וְלֹא","and not"],
    ["יִכָּשְׁלוּ","shall fail"],
    ["כִּי","for"],
    ["יַשִּׂיגוּהָ׃","they shall obtain it"]
  ]},
  { num: "לב", words: [
    ["אַךְ","but"],
    ["כׇּל","all"],
    ["הַדְּבָרִים","the things"],
    ["יָבֹאוּ","must come to pass"],
    ["בְּעִתָּם׃","in their time"]
  ]},
  { num: "לג", words: [
    ["לָכֵן","wherefore"],
    ["אַל","not"],
    ["תִּלְאוּ","be weary"],
    ["בַּעֲשׂוֹת","in doing"],
    ["טוֹב","good"],
    ["כִּי","for"],
    ["אַתֶּם","you"],
    ["מַנִּיחִים","are laying"],
    ["יְסוֹד","the foundation of"],
    ["מִפְעָל","a work"],
    ["גָּדוֹל","great"],
    ["וּמִדְּבָרִים","and out of things"],
    ["קְטַנִּים","small"],
    ["יוֹצֵא","proceeds"],
    ["הַגָּדוֹל׃","that which is great"]
  ]},
  { num: "לד", words: [
    ["הִנֵּה","behold"],
    ["יְהוָה","the Lord"],
    ["דּוֹרֵשׁ","requires"],
    ["הַלֵּב","the heart"],
    ["וְנֶפֶשׁ","and a mind"],
    ["חֲפֵצָה","willing"],
    ["וְהַחָפֵץ","and the willing"],
    ["וְהַשּׁוֹמֵעַ","and the obedient"],
    ["יֹאכַל","shall eat"],
    ["טוּב","the good of"],
    ["אֶרֶץ","the land of"],
    ["צִיּוֹן","Zion"],
    ["בְּאַחֲרִית","in the latter part of"],
    ["הַיָּמִים","the days"],
    ["הָאֵלֶּה׃","these"]
  ]},
  { num: "לה", words: [
    ["וְהַמּוֹרְדִים","and the rebellious"],
    ["יִכָּרְתוּ","shall be cut off"],
    ["מֵאֶרֶץ","out of the land of"],
    ["צִיּוֹן","Zion"],
    ["וִישֻׁלְּחוּ","and shall be sent away"],
    ["וְלֹא","and not"],
    ["יִירְשׁוּ","they shall inherit"],
    ["הָאָרֶץ׃","the land"]
  ]},
  { num: "לו", words: [
    ["כִּי","for"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["הַמּוֹרְדִים","the rebellious"],
    ["אֵינָם","are not"],
    ["מִדַּם","of the blood of"],
    ["אֶפְרַיִם","Ephraim"],
    ["לָכֵן","wherefore"],
    ["יִנָּתְשׁוּ׃","they shall be plucked out"]
  ]},
  { num: "לז", words: [["הִנֵּה","behold"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["עָשִׂיתִי","have made"], ["כְּנֵסִיָּתִי","my church"], ["בְּאַחֲרִית","in the latter part of"], ["הַיָּמִים","the days"], ["הָאֵלֶּה","these"], ["כְּשׁוֹפֵט","like a judge"], ["יוֹשֵׁב","sitting"], ["עַל","upon"], ["הַר","a hill"], ["אוֹ","or"], ["בְּמָקוֹם","in a place"], ["גָּבוֹהַ","high"], ["לִשְׁפֹּט","to judge"], ["הַגּוֹיִם׃","the nations"]]},
  { num: "לח", words: [
    ["כִּי","for"],
    ["יָבוֹא","it shall come to pass"],
    ["כִּי","that"],
    ["יוֹשְׁבֵי","the inhabitants of"],
    ["צִיּוֹן","Zion"],
    ["יִשְׁפְּטוּ","shall judge"],
    ["כׇּל","all"],
    ["דָּבָר","thing"],
    ["הַנּוֹגֵעַ","pertaining"],
    ["לְצִיּוֹן׃","to Zion"]
  ]},
  { num: "לט", words: [
    ["וַחֲנֵפִים","and hypocrites"],
    ["וּמְכַזְּבִים","and liars"],
    ["יִבָּחֲנוּ","shall be proved"],
    ["בָהֶם","by them"],
    ["וַאֲשֶׁר","and those who"],
    ["אֵינָם","are not"],
    ["שְׁלוּחִים","apostles"],
    ["וּנְבִיאִים","and prophets"],
    ["יִוָּדָעוּ׃","shall be known"]
  ]},
  { num: "מ", words: [
    ["וְגַם","and even"],
    ["הַפָּקִיד","the bishop"],
    ["אֲשֶׁר","who"],
    ["הוּא","is"],
    ["שׁוֹפֵט","a judge"],
    ["וְיוֹעֲצָיו","and his counselors"],
    ["אִם","if"],
    ["אֵינָם","they are not"],
    ["נֶאֱמָנִים","faithful"],
    ["יֻרְשְׁעוּ","shall be condemned"],
    ["וַאֲחֵרִים","and others"],
    ["יִנָּטְעוּ","shall be planted"],
    ["בִּמְקוֹמָם׃","in their stead"]
  ]},
  { num: "מא", words: [
    ["כִּי","for"],
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["צִיּוֹן","Zion"],
    ["תִּפְרַח","shall flourish"],
    ["וּכְבוֹד","and the glory of"],
    ["יְהוָה","the Lord"],
    ["יִהְיֶה","shall be"],
    ["עָלֶיהָ׃","upon her"]
  ]},
  { num: "מב", words: [
    ["וְתִהְיֶה","and she shall be"],
    ["לְנֵס","an ensign"],
    ["לָעָם","unto the people"],
    ["וְיָבֹאוּ","and shall come"],
    ["אֵלֶיהָ","unto her"],
    ["מִכׇּל","out of every"],
    ["גּוֹי","nation"],
    ["תַּחַת","under"],
    ["הַשָּׁמָיִם׃","heaven"]
  ]},
  { num: "מג", words: [
    ["וְיָבוֹא","and shall come"],
    ["הַיּוֹם","the day"],
    ["אֲשֶׁר","when"],
    ["גּוֹיֵי","the nations of"],
    ["הָאָרֶץ","the earth"],
    ["יֶחֶרְדוּ","shall tremble"],
    ["מִפָּנֶיהָ","because of her"],
    ["וְיִירְאוּ","and shall fear"],
    ["מִפְּנֵי","because of"],
    ["גִּבּוֹרֶיהָ","her terrible ones"],
    ["יְהוָה","the Lord"],
    ["דִּבֵּר","has spoken it"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc64_ch1Verses, 'dc64-ch1-verses');



var dc65_ch1Verses = [
  { num: "א", words: [
    ["הַאֲזִינוּ","hearken"],
    ["וְהִנֵּה","and lo"],
    ["קוֹל","a voice"],
    ["כְּשָׁלוּחַ","as of one sent down"],
    ["מִמָּרוֹם","from on high"],
    ["אַדִּיר","mighty"],
    ["וְחָזָק","and powerful"],
    ["אֲשֶׁר","whose"],
    ["מוֹצָאוֹ","going forth is"],
    ["לְקַצְוֵי","unto the ends of"],
    ["הָאָרֶץ","the earth"],
    ["אָכֵן","yea"],
    ["אֲשֶׁר","whose"],
    ["קוֹלוֹ","voice"],
    ["אֶל","is unto"],
    ["בְּנֵי","the sons of"],
    ["אָדָם","men"],
    ["פַּנּוּ","prepare"],
    ["דֶּרֶךְ","the way of"],
    ["יְהוָה","the Lord"],
    ["יַשְּׁרוּ","make straight"],
    ["מְסִלּוֹתָיו׃","his paths"]
  ]},
  { num: "ב", words: [
    ["מַפְתְּחוֹת","the keys of"],
    ["מַלְכוּת","the kingdom of"],
    ["אֱלֹהִים","God"],
    ["נִמְסְרוּ","are committed"],
    ["לְאָדָם","unto man"],
    ["עַל","upon"],
    ["הָאָרֶץ","the earth"],
    ["וּמִשָּׁם","and from thence"],
    ["תִּתְגֹּלֵל","shall roll forth"],
    ["הַבְּשׂוֹרָה","the gospel"],
    ["לְקַצְוֵי","unto the ends of"],
    ["הָאָרֶץ","the earth"],
    ["כָּאֶבֶן","as the stone"],
    ["אֲשֶׁר","which"],
    ["נִגְזְרָה","is cut"],
    ["מִן","out of"],
    ["הָהָר","the mountain"],
    ["לֹא","not"],
    ["בְיָדַיִם","by hands"],
    ["תִּתְגֹּלֵל","shall roll forth"],
    ["עַד","until"],
    ["אֲשֶׁר","that"],
    ["מָלְאָה","it has filled"],
    ["כׇּל","the whole"],
    ["הָאָרֶץ׃","earth"]
  ]},
  { num: "ג", words: [
    ["אָכֵן","yea"],
    ["קוֹל","a voice"],
    ["קוֹרֵא","crying"],
    ["פַּנּוּ","prepare"],
    ["דֶּרֶךְ","the way of"],
    ["יְהוָה","the Lord"],
    ["הָכִינוּ","prepare"],
    ["סְעוּדַת","the supper of"],
    ["הַשֶּׂה","the Lamb"],
    ["הִכּוֹנוּ","make ready"],
    ["לֶחָתָן׃","for the Bridegroom"]
  ]},
  { num: "ד", words: [
    ["הִתְפַּלְּלוּ","pray"],
    ["אֶל","unto"],
    ["יְהוָה","the Lord"],
    ["קִרְאוּ","call"],
    ["בְּשֵׁם","upon the name of"],
    ["קׇדְשׁוֹ","his holiness"],
    ["הוֹדִיעוּ","make known"],
    ["נִפְלְאוֹתָיו","his wonderful works"],
    ["בָּעַמִּים׃","among the people"]
  ]},
  { num: "ה", words: [
    ["קִרְאוּ","call"],
    ["אֶל","upon"],
    ["יְהוָה","the Lord"],
    ["לְמַעַן","that"],
    ["תֵּלֵךְ","may go forth"],
    ["מַלְכוּתוֹ","his kingdom"],
    ["עַל","upon"],
    ["הָאָרֶץ","the earth"],
    ["לְמַעַן","that"],
    ["יְקַבְּלוּהָ","may receive it"],
    ["יוֹשְׁבֶיהָ","its inhabitants"],
    ["וְיֻכְּנוּ","and be prepared"],
    ["לַיָּמִים","for the days"],
    ["הַבָּאִים","to come"],
    ["בָּהֶם","in which"],
    ["יֵרֵד","shall come down"],
    ["בֶּן","the Son of"],
    ["הָאָדָם","Man"],
    ["מִן","from"],
    ["הַשָּׁמַיִם","heaven"],
    ["עָטוּי","clothed"],
    ["בְּזֹהַר","in the brightness of"],
    ["כְּבוֹדוֹ","his glory"],
    ["לִקְרַאת","to meet"],
    ["מַלְכוּת","the kingdom of"],
    ["אֱלֹהִים","God"],
    ["עַל","upon"],
    ["הָאָרֶץ׃","the earth"]
  ]},
  { num: "ו", words: [
    ["לָכֵן","wherefore"],
    ["תֵּלֵךְ","may go forth"],
    ["מַלְכוּת","the kingdom of"],
    ["אֱלֹהִים","God"],
    ["לְמַעַן","that"],
    ["תָּבוֹא","may come"],
    ["מַלְכוּת","the kingdom of"],
    ["הַשָּׁמַיִם","the heavens"],
    ["לְמַעַן","that"],
    ["תִּכָּבֵד","you may be glorified"],
    ["אֱלֹהִים","O God"],
    ["בַּשָּׁמַיִם","in heaven"],
    ["כֵּן","so"],
    ["בָּאָרֶץ","on earth"],
    ["לְמַעַן","that"],
    ["יִכָּנְעוּ","may be subdued"],
    ["אֹיְבֶיךָ","your enemies"],
    ["כִּי","for"],
    ["לְךָ","your is"],
    ["הַהָדָר","the honor"],
    ["וְהָעֹז","and the power"],
    ["וְהַכָּבוֹד","and the glory"],
    ["לְעוֹלָם","for ever"],
    ["וָעֶד","and ever"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc65_ch1Verses, 'dc65-ch1-verses');



var dc66_ch1Verses = [
  { num: "א", words: [
    ["הִנֵּה","behold"],
    ["כֹּה","thus"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["לְעַבְדִּי","unto my servant"],
    ["וִילְיָם","William"],
    ["אֶי.","E."],
    ["מַקְלֵלִין","McLellin"],
    ["אַשְׁרֶיךָ","blessed are you"],
    ["בַּאֲשֶׁר","inasmuch as"],
    ["שַׁבְתָּ","you have turned away"],
    ["מֵעֲוֹנוֹתֶיךָ","from your iniquities"],
    ["וְקִבַּלְתָּ","and have received"],
    ["אֲמִתִּי","my truths"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["גֹּאֲלֶךָ","your Redeemer"],
    ["מוֹשִׁיעַ","the Savior of"],
    ["הָעוֹלָם","the world"],
    ["אַף","even"],
    ["כׇּל","all"],
    ["הַמַּאֲמִינִים","as believe"],
    ["בִּשְׁמִי׃","my name"]
  ]},
  { num: "ב", words: [["אָמֵן","verily"], ["אֲנִי","I"], ["אֹמֵר","say"], ["לְךָ","unto you"], ["אַשְׁרֶיךָ","blessed are you"], ["עַל","for"], ["קַבֶּלְךָ","your receiving"], ["בְּרִיתִי","my covenant"], ["בְּרִית־עוֹלָם","an everlasting covenant"], ["אַף","even"], ["מְלֹא","the fulness of"], ["בְּשׂוֹרָתִי","my gospel"], ["הַשְּׁלוּחָה","sent forth"], ["לִבְנֵי","unto the children of"], ["הָאָדָם","men"], ["לְמַעַן","that"], ["יִהְיוּ","there might be"], ["לָהֶם","unto them"], ["חַיִּים","life"], ["וְיִהְיוּ","and be"], ["חֲבֵרִים","partakers"], ["לַתִּפְאֲרוֹת","of the glories"], ["הַנִּגְלוֹת","which are revealed"], ["בְּאַחֲרִית","in the latter part of"], ["הַיָּמִים","the days"], ["כַּאֲשֶׁר","as"], ["נִכְתַּב","was written"], ["בִּידֵי","by the hands of"], ["הַנְּבִיאִים","the prophets"], ["וְהַשְּׁלוּחִים","and the apostles"], ["בִּימֵי","in days of"], ["קֶדֶם׃","old"]]},
  { num: "ג", words: [
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לְךָ","unto you"],
    ["עַבְדִּי","my servant"],
    ["וִילְיָם","William"],
    ["אַתָּה","you are"],
    ["טָהוֹר","clean"],
    ["אַךְ","but"],
    ["לֹא","not"],
    ["כֻלְּךָ","all"],
    ["שׁוּב","repent"],
    ["לָכֵן","therefore"],
    ["מֵהַדְּבָרִים","of the things"],
    ["אֲשֶׁר","which"],
    ["אֵינָם","are not"],
    ["רְצוּיִים","pleasing"],
    ["בְּעֵינַי","in my sight"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["כִּי","for"],
    ["יְהוָה","the Lord"],
    ["יַרְאֶה","will show"],
    ["אוֹתָם","them"],
    ["לְךָ׃","unto you"]
  ]},
  { num: "ד", words: [
    ["וְעַתָּה","and now"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אַרְאֶה","will show"],
    ["לְךָ","unto you"],
    ["אֲשֶׁר","what"],
    ["אֲנִי","I"],
    ["חָפֵץ","will"],
    ["עָלֶיךָ","concerning you"],
    ["אוֹ","or"],
    ["מַה","what"],
    ["רְצוֹנִי","is my will"],
    ["עָלֶיךָ׃","concerning you"]
  ]},
  { num: "ה", words: [
    ["הִנֵּה","behold"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לְךָ","unto you"],
    ["רְצוֹנִי","my will is"],
    ["כִּי","that"],
    ["תַּכְרִיז","you should proclaim"],
    ["בְּשׂוֹרָתִי","my gospel"],
    ["מֵאֶרֶץ","from land"],
    ["לְאֶרֶץ","to land"],
    ["וּמֵעִיר","and from city"],
    ["לְעִיר","to city"],
    ["אָכֵן","yea"],
    ["בַּגְּלִילוֹת","in the regions"],
    ["סָבִיב","round about"],
    ["אֲשֶׁר","where"],
    ["לֹא","not"],
    ["הֻכְרְזָה׃","it has been proclaimed"]
  ]},
  { num: "ו", words: [
    ["אַל","do not"],
    ["תִּתְמַהְמַהּ","tarry"],
    ["יָמִים","days"],
    ["רַבִּים","many"],
    ["בַּמָּקוֹם","in place"],
    ["הַזֶּה","this"],
    ["אַל","not"],
    ["תַּעֲלֶה","go up"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["עַד־כֹּה","as yet"],
    ["אַךְ","but"],
    ["כַּאֲשֶׁר","inasmuch as"],
    ["תּוּכַל","you can"],
    ["שְׁלָח","send"],
    ["וְאִם","and if"],
    ["לֹא","not"],
    ["אַל","not"],
    ["תַּחֲשֹׁב","think"],
    ["עַל","of"],
    ["רְכוּשְׁךָ׃","your property"]
  ]},
  { num: "ז", words: [
    ["לֵךְ","go"],
    ["לְאַרְצוֹת","unto the lands of"],
    ["הַמִּזְרָח","the east"],
    ["הָעֵד","bear testimony"],
    ["בְּכׇל","in every"],
    ["מָקוֹם","place"],
    ["לְכׇל","unto every"],
    ["עָם","people"],
    ["וּבְבָתֵּי","and in the houses of"],
    ["כְנֵסִיּוֹתֵיהֶם","their synagogues"],
    ["וְהִתְוַכֵּחַ","reasoning"],
    ["עִם","with"],
    ["הָעָם׃","the people"]
  ]},
  { num: "ח", words: [
    ["עַבְדִּי","my servant"],
    ["שְׁמוּאֵל","Samuel"],
    ["ה","H."],
    ["סְמִית","Smith"],
    ["יֵלֵךְ","let go"],
    ["עִמְּךָ","with you"],
    ["וְאַל","and do not"],
    ["תַּעַזְבֶנּוּ","forsake him"],
    ["וְתֵן","and give"],
    ["לוֹ","him"],
    ["תּוֹרֹתֶיךָ","your instructions"],
    ["וְהַנֶּאֱמָן","and he that is faithful"],
    ["יְחֻזַּק","shall be made strong"],
    ["בְּכׇל","in every"],
    ["מָקוֹם","place"],
    ["וַאֲנִי","and I"],
    ["יְהוָה","the Lord"],
    ["אֵלֵךְ","will go"],
    ["עִמְּךָ׃","with you"]
  ]},
  { num: "ט", words: [["שִׂים","lay"], ["יָדֶיךָ","your hands"], ["עַל","upon"], ["הַחוֹלִים","the sick"], ["וְיֵרָפְאוּ","and they shall recover"], ["אַל","do not"], ["תָּשֹׁב","return"], ["עַד","until"], ["אֲשֶׁר","that"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["אֶשְׁלָחֲךָ","shall send you"], ["הֱיֵה","be"], ["אֶרֶךְ־אַפַּיִם","longsuffering"], ["בְּצָרָה","in affliction"], ["שְׁאַל","ask"], ["וְיִנָּתֶן","and it shall be given"], ["לְךָ","unto you"], ["דְּפֹק","knock"], ["וְיִפָּתַח","and it shall be opened"], ["לְךָ׃","unto you"]]},
  { num: "י", words: [
    ["אַל","do not"],
    ["תְּבַקֵּשׁ","seek"],
    ["לְהִטָּרֵד","to be cumbered"],
    ["עֲזֹב","forsake"],
    ["כׇּל","all"],
    ["עַוְלָה","unrighteousness"],
    ["אַל","do not"],
    ["תִּנְאַף","commit adultery"],
    ["נִסָּיוֹן","a temptation"],
    ["אֲשֶׁר","with which"],
    ["צֹרַפְתָּ׃","you have been troubled"]
  ]},
  { num: "יא", words: [
    ["שְׁמֹר","keep"],
    ["הָאֲמָרִים","the sayings"],
    ["הָאֵלֶּה","these"],
    ["כִּי","for"],
    ["אֱמֶת","truth"],
    ["וֶאֱמוּנָה","and faithfulness"],
    ["הֵם","they are"],
    ["מִשְׂרָתְךָ","your office"],
    ["וְתִנְגַּח","and you shall push"],
    ["עַמִּים","peoples"],
    ["רַבִּים","many"],
    ["לְצִיּוֹן","to Zion"],
    ["בְּשִׁירֵי","with songs of"],
    ["שִׂמְחַת","the joy of"],
    ["עוֹלָם","everlasting"],
    ["עַל","upon"],
    ["רֹאשָׁם׃","their heads"]
  ]},
  { num: "יב", words: [
    ["עֲמֹד","continue"],
    ["בַּדְּבָרִים","in the things"],
    ["הָאֵלֶּה","these"],
    ["עַד","unto"],
    ["הַקֵּץ","the end"],
    ["וִיהִי","and there shall be"],
    ["לְךָ","unto you"],
    ["עֲטֶרֶת","a crown of"],
    ["חַיֵּי","life of"],
    ["עוֹלָם","eternal"],
    ["לִימִין","at the right hand of"],
    ["אָבִי","my Father"],
    ["הַמָּלֵא","who is full of"],
    ["חֶסֶד","grace"],
    ["וֶאֱמֶת׃","and truth"]
  ]},
  { num: "יג", words: [
    ["אָמֵן","verily"],
    ["כֹּה","thus"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["אֱלֹהֶיךָ","your God"],
    ["גֹּאַלְךָ","your Redeemer"],
    ["יֵשׁוּעַ","even Jesus"],
    ["הַמָּשִׁיחַ","Christ"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc66_ch1Verses, 'dc66-ch1-verses');



var dc67_ch1Verses = [
  { num: "א", words: [["הִנֵּה","behold"], ["וְהַאֲזִינוּ","and hearken"], ["זִקְנֵי","O elders of"], ["כְּנֵסִיָּתִי","my church"], ["אֲשֶׁר","who"], ["נֶאֱסַפְתֶּם","have assembled yourselves"], ["יַחְדָּו","together"], ["אֲשֶׁר","whose"], ["שָׁמַעְתִּי","I have heard"], ["תְּפִלּוֹתֵיכֶם","your prayers"], ["וְיוֹדֵעַ","and know"], ["אֲנִי","I"], ["לִבְּכֶם","your hearts"], ["וּמִשְׁאֲלוֹתֵיכֶם","and your desires"], ["עָלוּ","have come up"], ["לְפָנָי׃","before me"]]},
  { num: "ב", words: [
    ["הִנֵּה","behold"],
    ["עֵינַי","my eyes are"],
    ["עֲלֵיכֶם","upon you"],
    ["וְהַשָּׁמַיִם","and the heavens"],
    ["וְהָאָרֶץ","and the earth"],
    ["בְּיָדַי","are in my hands"],
    ["וְעֹשֶׁר","and the riches of"],
    ["הַנֶּצַח","eternity"],
    ["לִי","are mine"],
    ["לָתֵת׃","to give"]
  ]},
  { num: "ג", words: [
    ["נִסִּיתֶם","you endeavored"],
    ["לְהַאֲמִין","to believe"],
    ["כִּי","that"],
    ["תְּקַבְּלוּ","you should receive"],
    ["הַבְּרָכָה","the blessing"],
    ["אֲשֶׁר","which"],
    ["הֻגְּשָׁה","was offered"],
    ["לָכֶם","unto you"],
    ["אַךְ","but"],
    ["הִנֵּה","behold"],
    ["אָמֵן","verily"],
    ["אֲנִי","I"],
    ["אֹמֵר","say"],
    ["לָכֶם","unto you"],
    ["הָיוּ","there were"],
    ["פְחָדִים","fears"],
    ["בִּלְבַבְכֶם","in your hearts"],
    ["וְאָמֵן","and verily"],
    ["זֹאת","this is"],
    ["הַסִּבָּה","the reason"],
    ["אֲשֶׁר","that"],
    ["לֹא","not"],
    ["קִבַּלְתֶּם׃","you did receive"]
  ]},
  { num: "ד", words: [
    ["וְעַתָּה","and now"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["נוֹתֵן","give"],
    ["לָכֶם","unto you"],
    ["עֵדוּת","a testimony"],
    ["עַל","of"],
    ["אֱמֶת","the truth of"],
    ["הַמִּצְוֹת","the commandments"],
    ["הָאֵלֶּה","these"],
    ["הַמֻּנָּחוֹת","which are lying"],
    ["לִפְנֵיכֶם׃","before you"]
  ]},
  { num: "ה", words: [["עֵינֵיכֶם","your eyes"], ["הָיוּ","have been"], ["עַל","upon"], ["עַבְדִּי","my servant"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["וּשְׂפָתוֹ","and his language"], ["יְדַעְתֶּם","you have known"], ["וּמוּמָיו","and his imperfections"], ["יְדַעְתֶּם","you have known"], ["וּבִקַּשְׁתֶּם","and you have sought"], ["בִּלְבַבְכֶם","in your hearts"], ["דַּעַת","knowledge"], ["לְבַטֵּא","to express"], ["יוֹתֵר","beyond"], ["מִשְּׂפָתוֹ","his language"], ["וְזֹאת","and this"], ["גַּם","also"], ["יְדַעְתֶּם׃","you know"]]},
  { num: "ו", words: [
    ["עַתָּה","now"],
    ["חַפְּשׂוּ","seek you out"],
    ["מִסֵּפֶר","of the Book of"],
    ["הַמִּצְוֹת","the Commandments"],
    ["גַּם","even"],
    ["הַקָּטָן","the least"],
    ["אֲשֶׁר","that is"],
    ["בָּהֶם","among them"],
    ["וּמַנּוּ","and appoint"],
    ["אֵת־הֶחָכָם","the most wise"],
    ["בָּכֶם׃","among you"]
  ]},
  { num: "ז", words: [
    ["אוֹ","or"],
    ["אִם","if"],
    ["יֵשׁ","there be"],
    ["בָּכֶם","among you"],
    ["אֲשֶׁר","one who"],
    ["יַעֲשֶׂה","shall make"],
    ["אֶחָד","one"],
    ["כָּמוֹהוּ","like unto it"],
    ["אָז","then"],
    ["אַתֶּם","you"],
    ["נִצְדָּקִים","are justified"],
    ["לֵאמֹר","in saying"],
    ["כִּי","that"],
    ["אֵינְכֶם","you do not"],
    ["יוֹדְעִים","know"],
    ["כִּי","that"],
    ["אֱמֶת","true"],
    ["הֵם׃","they are"]
  ]},
  { num: "ח", words: [
    ["אַךְ","but"],
    ["אִם","if"],
    ["אֵינְכֶם","you are not"],
    ["יְכוֹלִים","able"],
    ["לַעֲשׂוֹת","to make"],
    ["אֶחָד","one"],
    ["כָּמוֹהוּ","like unto it"],
    ["אַתֶּם","you are"],
    ["תַּחַת","under"],
    ["אַשְׁמָה","condemnation"],
    ["אִם","if"],
    ["אֵינְכֶם","you do not"],
    ["מְעִידִים","bear record"],
    ["כִּי","that"],
    ["אֱמֶת","true"],
    ["הֵם׃","they are"]
  ]},
  { num: "ט", words: [
    ["כִּי","for"],
    ["יוֹדְעִים","know"],
    ["אַתֶּם","you"],
    ["כִּי","that"],
    ["אֵין","there is no"],
    ["בָּהֶם","in them"],
    ["עַוְלָה","unrighteousness"],
    ["וַאֲשֶׁר","and that which"],
    ["צַדִּיק","is righteous"],
    ["הוּא","it"],
    ["יוֹרֵד","comes down"],
    ["מִמַּעַל","from above"],
    ["מֵאֲבִי","from the Father of"],
    ["הָאוֹרוֹת׃","lights"]
  ]},
  { num: "י", words: [["וְעוֹד","and again"], ["אָמֵן","verily"], ["אֲנִי","I"], ["אֹמֵר","say"], ["לָכֶם","unto you"], ["זְכוּתְכֶם","it is your privilege"], ["הִיא","it"], ["וְהַבְטָחָה","and a promise"], ["אֲנִי","I"], ["נוֹתֵן","give"], ["לָכֶם","unto you"], ["אֲשֶׁר","who"], ["לָעֲבוֹדָה","unto ministry"], ["הַזֹּאת","this"], ["כִּי","that"], ["אִם","inasmuch as"], ["תִּפְשְׁטוּ","you strip"], ["מֵעֲלֵיכֶם","from you"], ["קִנְאוֹת","jealousies"], ["וּפְחָדִים","and fears"], ["וְתִכָּנְעוּ","and humble yourselves"], ["לְפָנַי","before me"], ["כִּי","for"], ["אֵינְכֶם","you are not"], ["עֲנָוִים","humble"], ["דַּי","sufficiently"], ["הַפָּרֹכֶת","the veil"], ["תִּקָּרַע","shall be rent"], ["וְתִרְאוּנִי","and you shall see me"], ["וְתֵדְעוּ","and you shall know"], ["כִּי","that"], ["אֲנִי","I"], ["הוּא","am"], ["לֹא","not"], ["בַּדַּעַת","with the mind"], ["הַבְּשָׂרִית","carnal"], ["וְלֹא","neither"], ["הַנַּפְשִׁית","natural"], ["כִּי־אִם","but"], ["בָּרוּחָנִית׃","with the spiritual"]]},
  { num: "יא", words: [
    ["כִּי","for"],
    ["לֹא","not"],
    ["רָאָה","has seen"],
    ["אָדָם","man"],
    ["אֶת־אֱלֹהִים","God"],
    ["מֵעוֹלָם","at any time"],
    ["בַּבָּשָׂר","in the flesh"],
    ["בִּלְתִּי","except"],
    ["אִם","if"],
    ["כֵּן","so"],
    ["הֶחֱיָהוּ","quickened him"],
    ["רוּחַ","the Spirit of"],
    ["אֱלֹהִים׃","God"]
  ]},
  { num: "יב", words: [
    ["וְגַם","and also"],
    ["לֹא","not"],
    ["יָכוֹל","can"],
    ["אָדָם","any man"],
    ["נַפְשִׁי","natural"],
    ["לִשְׂאֵת","abide"],
    ["פְּנֵי","the presence of"],
    ["אֱלֹהִים","God"],
    ["וְלֹא","neither"],
    ["לְפִי","after"],
    ["דַּעַת","the mind"],
    ["הַבָּשָׂר׃","carnal"]
  ]},
  { num: "יג", words: [["אֵינְכֶם","you are not"], ["יְכוֹלִים","able"], ["לִשְׂאֵת","to abide"], ["פְּנֵי","the presence of"], ["אֱלֹהִים","God"], ["עַתָּה","now"], ["וְלֹא","neither"], ["שָׁרֵת","the ministering of"], ["הַמַּלְאָכִים","the angels"], ["לָכֵן","wherefore"], ["עִמְדוּ","continue"], ["בְּאֹרֶךְ־אַפַּיִם","in patience"], ["עַד","until"], ["אֲשֶׁר","that"], ["תֻּשְׁלָמוּ׃","you are perfected"]]},
  { num: "יד", words: [["אַל","let not"], ["יִפֶן","turn"], ["לְבַבְכֶם","your minds"], ["אָחוֹר","back"], ["וְכַאֲשֶׁר","and when"], ["תִּהְיוּ","you shall be"], ["רְאוּיִים","worthy"], ["בְּעִתִּי","in my time"], ["הָרְאוּיָה","due"], ["תִּרְאוּ","you shall see"], ["וְתֵדְעוּ","and you shall know"], ["אֵת־אֲשֶׁר","that which"], ["נִמְסַר","was conferred"], ["לָכֶם","unto you"], ["בְּיַד","by the hands of"], ["עַבְדִּי","my servant"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["אָמֵן׃","amen"]]}
];
renderVerseSet(dc67_ch1Verses, 'dc67-ch1-verses');



var dc68_ch1Verses = [
  { num: "א", words: [
    ["עַבְדִּי","my servant"],
    ["אוֹרְסוֹן","Orson"],
    ["הַיְד","Hyde"],
    ["נִקְרָא","was called"],
    ["בִּסְמִיכָתוֹ","by his ordination"],
    ["לְהַכְרִיז","to proclaim"],
    ["בְּשׂוֹרַת","the gospel of"],
    ["עוֹלָם","everlasting"],
    ["בְּרוּחַ","by the Spirit of"],
    ["אֱלֹהִים","the God"],
    ["חַיִּים","living"],
    ["מֵעָם","from people"],
    ["לְעָם","to people"],
    ["וּמֵאֶרֶץ","and from land"],
    ["לְאֶרֶץ","to land"],
    ["בְּקֶרֶב","in the midst of"],
    ["קְהִלּוֹת","the congregations of"],
    ["הָרְשָׁעִים","the wicked"],
    ["בְּבָתֵּי","in the houses of"],
    ["כְנֵסִיּוֹתֵיהֶם","their synagogues"],
    ["מִתְוַכֵּחַ","reasoning"],
    ["וּמְבָאֵר","and expounding"],
    ["לָהֶם","unto them"],
    ["כׇּל","all"],
    ["הַכְּתוּבִים׃","the scriptures"]
  ]},
  { num: "ב", words: [
    ["וְהִנֵּה","and behold"],
    ["זֶה","this is"],
    ["מוֹפֵת","an ensample"],
    ["לְכׇל","unto all"],
    ["אֵלֶּה","those"],
    ["אֲשֶׁר","who"],
    ["נִסְמְכוּ","were ordained"],
    ["לַכְּהֻנָּה","unto the priesthood"],
    ["הַזֹּאת","this"],
    ["אֲשֶׁר","whose"],
    ["מִשְׁלַחְתָּם","mission"],
    ["מֻנָּה","is appointed"],
    ["לָהֶם","unto them"],
    ["לָצֵאת׃","to go forth"]
  ]},
  { num: "ג", words: [["וְזֶה","and this is"], ["הַמּוֹפֵת","the ensample"], ["לָהֶם","unto them"], ["כִּי","that"], ["יְדַבְּרוּ","they shall speak"], ["כַּאֲשֶׁר","as"], ["יִנָּשְׂאוּ","they are moved upon"], ["בְּיַד","by the means of"], ["רוּחַ","the Spirit of"], ["הַקֹּדֶשׁ׃","the Holy"]]},
  { num: "ד", words: [["וְכׇל","and whatsoever"], ["אֲשֶׁר","that"], ["יְדַבְּרוּ","they shall speak"], ["בְּהִנָּשְׂאָם","when moved upon"], ["בְּיַד","by the means of"], ["רוּחַ","the Spirit of"], ["הַקֹּדֶשׁ","the Holy"], ["יִהְיֶה","shall be"], ["כָּתוּב","scripture"], ["יִהְיֶה","shall be"], ["רְצוֹן","the will of"], ["יְהוָה","the Lord"], ["יִהְיֶה","shall be"], ["דַּעַת","the mind of"], ["יְהוָה","the Lord"], ["יִהְיֶה","shall be"], ["דְּבַר","the word of"], ["יְהוָה","the Lord"], ["יִהְיֶה","shall be"], ["קוֹל","the voice of"], ["יְהוָה","the Lord"], ["וּגְבוּרַת","and the power of"], ["אֱלֹהִים","God"], ["לִישׁוּעָה׃","unto salvation"]]},
  { num: "ה", words: [
    ["הִנֵּה","behold"],
    ["זֹאת","this is"],
    ["הַבְטָחַת","the promise of"],
    ["יְהוָה","the Lord"],
    ["לָכֶם","unto you"],
    ["עֲבָדַי׃","O my servants"]
  ]},
  { num: "ו", words: [
    ["לָכֵן","wherefore"],
    ["הֱיוּ","be"],
    ["שְׂמֵחִים","of good cheer"],
    ["וְאַל","and do not"],
    ["תִּירְאוּ","fear"],
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["עִמָּכֶם","am with you"],
    ["וְאֶעֱמֹד","and I will stand"],
    ["לְצִדְּכֶם","by you"],
    ["וְתָעִידוּ","and you shall bear record"],
    ["עָלַי","of me"],
    ["יֵשׁוּעַ","even Jesus"],
    ["הַמָּשִׁיחַ","Christ"],
    ["כִּי","that"],
    ["אֲנִי","I am"],
    ["בֶּן","the Son of"],
    ["אֱלֹהִים","the God"],
    ["חַיִּים","living"],
    ["כִּי","that"],
    ["הָיִיתִי","I was"],
    ["וַאֲנִי","and I"],
    ["הוֹוֶה","am"],
    ["וַאֲנִי","and I"],
    ["עָתִיד","am to"],
    ["לָבוֹא׃","come"]
  ]},
  { num: "ז", words: [["זֶה","this is"], ["דְּבַר","the word of"], ["יְהוָה","the Lord"], ["אֲלֵיכֶם","unto you"], ["עַבְדִּי","my servant"], ["אוֹרְסוֹן","Orson"], ["הַיְד","Hyde"], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["לוּק","Luke"], ["ג׳וֹנְסוֹן","Johnson"], ["וּלְעַבְדִּי","and unto my servant"], ["לִימָן","Lyman"], ["ג׳וֹנְסוֹן","Johnson"], ["וּלְעַבְדִּי","and unto my servant"], ["וִילְיָם","William"], ["אֶי.","E."], ["מַקְלֵלִין","McLellin"], ["וּלְכׇל","and unto all"], ["זִקְנֵי","the elders of"], ["כְּנֵסִיָּתִי","my church"], ["הַנֶּאֱמָנִים׃","the faithful"]]},
  { num: "ח", words: [["לְכוּ","go"], ["לְכׇל","into all"], ["הָעוֹלָם","the world"], ["קִרְאוּ","preach"], ["הַבְּשׂוֹרָה","the gospel"], ["לְכׇל","unto every"], ["בְּרִיאָה","creature"], ["בִּפְעָלְכֶם","acting"], ["בַּשִּׁלְטוֹן","in the authority"], ["אֲשֶׁר","which"], ["נָתַתִּי","I have given"], ["לָכֶם","unto you"], ["מַטְבִּילִים","baptizing"], ["בְּשֵׁם","in the name of"], ["הָאָב","the Father"], ["וְהַבֵּן","and the Son"], ["וְרוּחַ","and the Spirit of"], ["הַקֹּדֶשׁ׃","the Holy"]]},
  { num: "ט", words: [
    ["וְהַמַּאֲמִין","and he that believes"],
    ["וְהַנִּטְבָּל","and is baptized"],
    ["יִוָּשַׁע","shall be saved"],
    ["וַאֲשֶׁר","and he who"],
    ["אֵינֶנּוּ","not"],
    ["מַאֲמִין","believes"],
    ["יֶאְשַׁם׃","shall be damned"]
  ]},
  { num: "י", words: [
    ["וְהַמַּאֲמִין","and he that believes"],
    ["יְבֹרַךְ","shall be blest"],
    ["בְּאוֹתוֹת","with signs"],
    ["הוֹלְכִים","following"],
    ["אַחֲרָיו","after him"],
    ["כַּכָּתוּב׃","even as it is written"]
  ]},
  { num: "יא", words: [
    ["וְלָכֶם","and unto you"],
    ["יִנָּתֵן","it shall be given"],
    ["לָדַעַת","to know"],
    ["אוֹתוֹת","the signs of"],
    ["הָעִתִּים","the times"],
    ["וְאוֹתוֹת","and the signs of"],
    ["בּוֹא","the coming of"],
    ["בֶּן","the Son of"],
    ["הָאָדָם׃","Man"]
  ]},
  { num: "יב", words: [
    ["וְכׇל","and of as many"],
    ["אֲשֶׁר","as"],
    ["יָעִיד","shall bear record"],
    ["עֲלֵיהֶם","of them"],
    ["הָאָב","the Father"],
    ["לָכֶם","unto you"],
    ["יִנָּתֵן","shall be given"],
    ["כֹּחַ","power"],
    ["לַחְתֹּם","to seal"],
    ["אוֹתָם","them"],
    ["לְחַיֵּי","up unto life"],
    ["עוֹלָם","eternal"],
    ["אָמֵן׃","amen"]
  ]},
  { num: "יג", words: [
    ["וְעַתָּה","and now"],
    ["עַל","concerning"],
    ["הַדְּבָרִים","the items"],
    ["הַנּוֹסָפִים","in addition"],
    ["עַל","to"],
    ["הַבְּרִיתוֹת","the covenants"],
    ["וְהַמִּצְוֹת","and the commandments"],
    ["אֵלֶּה","these"],
    ["הֵם׃","they are"]
  ]},
  { num: "יד", words: [["נִשְׁאָרִים","there remain"], ["אַחֲרֵי","after"], ["כֵן","this"], ["בְּעֵת","in the time of"], ["יְהוָה","the Lord"], ["הָרְאוּיָה","due"], ["פְּקִידִים","bishops"], ["אֲחֵרִים","other"], ["לְהִבָּדֵל","to be set apart"], ["לַכְּנֵסִיָּה","unto the church"], ["לְשָׁרֵת","to minister"], ["כְּמוֹ","even according to"], ["הָרִאשׁוֹן׃","the first"]]},
  { num: "טו", words: [
    ["לָכֵן","wherefore"],
    ["יִהְיוּ","they shall be"],
    ["כֹּהֲנִים","priests"],
    ["גְּדוֹלִים","high"],
    ["רְאוּיִים","worthy"],
    ["וִימֻנּוּ","and shall be appointed"],
    ["בְּיַד","by the means of"],
    ["נְשִׂיאוּת","the First Presidency of"],
    ["כְּהֻנַּת","the Priesthood of"],
    ["מַלְכִּיצֶדֶק","Melchizedek"],
    ["כִּי","except"],
    ["אִם","if"],
    ["יִהְיוּ","they be"],
    ["זֶרַע","the literal seed of"],
    ["אַהֲרֹן׃","Aaron"]
  ]},
  { num: "טז", words: [
    ["וְאִם","and if"],
    ["יִהְיוּ","they be"],
    ["זֶרַע","the literal seed of"],
    ["אַהֲרֹן","Aaron"],
    ["יֵשׁ","there is"],
    ["לָהֶם","unto them"],
    ["זְכוּת","a right"],
    ["כַּמִּשְׁפָּט","legal"],
    ["עַל","to"],
    ["הַפְּקִידוּת","the bishopric"],
    ["אִם","if"],
    ["הֵם","they are"],
    ["הַבְּכוֹרִים","the firstborn"],
    ["בִּבְנֵי","among the sons of"],
    ["אַהֲרֹן׃","Aaron"]
  ]},
  { num: "יז", words: [
    ["כִּי","for"],
    ["הַבְּכוֹר","the firstborn"],
    ["מַחֲזִיק","holds"],
    ["זְכוּת","the right of"],
    ["הַנְּשִׂיאוּת","the presidency"],
    ["עַל","over"],
    ["הַכְּהֻנָּה","the priesthood"],
    ["הַזֹּאת","this"],
    ["וְהַמַּפְתְּחוֹת","and the keys"],
    ["אוֹ","or"],
    ["הַשִּׁלְטוֹן","authority"],
    ["עָלֶיהָ׃","of the same"]
  ]},
  { num: "יח", words: [
    ["אֵין","there is no right"],
    ["לְאִישׁ","unto any man"],
    ["זְכוּת","right"],
    ["כַּמִּשְׁפָּט","legal"],
    ["עַל","to"],
    ["הַמִּשְׂרָה","the office"],
    ["הַזֶּה","this"],
    ["לְהַחֲזִיק","to hold"],
    ["מַפְתְּחוֹת","the keys of"],
    ["הַכְּהֻנָּה","the priesthood"],
    ["הַזֹּאת","this"],
    ["כִּי","except"],
    ["אִם","if"],
    ["יִהְיֶה","he be"],
    ["זֶרַע","the literal seed"],
    ["וּבְכוֹר","and the firstborn"],
    ["לְאַהֲרֹן׃","of Aaron"]
  ]},
  { num: "יט", words: [
    ["אַךְ","but"],
    ["בַּאֲשֶׁר","as"],
    ["לְכֹהֵן","to a priest"],
    ["גָּדוֹל","high"],
    ["בִּכְהֻנַּת","of the Priesthood of"],
    ["מַלְכִּיצֶדֶק","Melchizedek"],
    ["יֵשׁ","there is"],
    ["שִׁלְטוֹן","authority"],
    ["לְשָׁרֵת","to officiate"],
    ["בְּכׇל","in all"],
    ["הַמִּשְׂרוֹת","the lesser offices"],
    ["יוּכַל","he may"],
    ["לְשָׁרֵת","officiate"],
    ["בְּמִשְׂרַת","in the office of"],
    ["הַפָּקִיד","bishop"],
    ["כַּאֲשֶׁר","when"],
    ["לֹא","not"],
    ["נִמְצָא","can be found"],
    ["זֶרַע","the literal seed of"],
    ["אַהֲרֹן","Aaron"],
    ["אַךְ","provided"],
    ["אִם","that"],
    ["נִקְרָא","he is called"],
    ["וְנִבְדַּל","and set apart"],
    ["וְנִסְמַךְ","and ordained"],
    ["לַכֹּחַ","unto power"],
    ["הַזֶּה","this"],
    ["תַּחַת","under"],
    ["יְדֵי","the hands of"],
    ["נְשִׂיאוּת","the Presidency of"],
    ["כְּהֻנַּת","the Priesthood of"],
    ["מַלְכִּיצֶדֶק׃","Melchizedek"]
  ]},
  { num: "כ", words: [
    ["וְזֶרַע","and the literal seed of"],
    ["אַהֲרֹן","Aaron"],
    ["גַּם","also"],
    ["כֵּן","so"],
    ["עָלָיו","must"],
    ["לְהִתְמַנּוֹת","be designated"],
    ["בְּיַד","by the means of"],
    ["הַנְּשִׂיאוּת","the Presidency"],
    ["הַזֹּאת","this"],
    ["וּלְהִמָּצֵא","and to be found"],
    ["רָאוּי","worthy"],
    ["וּלְהִמָּשַׁח","and to be anointed"],
    ["וּלְהִסָּמֵךְ","and to be ordained"],
    ["תַּחַת","under"],
    ["יְדֵי","the hands of"],
    ["הַנְּשִׂיאוּת","the Presidency"],
    ["הַזֹּאת","this"],
    ["וְאִם","and if"],
    ["לֹא","not"],
    ["אֵינָם","they are not"],
    ["מֻרְשִׁים","authorized"],
    ["כַּמִּשְׁפָּט","legally"],
    ["לְשָׁרֵת","to officiate"],
    ["בִּכְהֻנָּתָם׃","in their priesthood"]
  ]},
  { num: "כא", words: [
    ["אַךְ","but"],
    ["מִכֹּחַ","by virtue of"],
    ["הַגְּזֵרָה","the decree"],
    ["עַל","concerning"],
    ["זְכוּת","the right of"],
    ["הַכְּהֻנָּה","the priesthood"],
    ["הַיּוֹרֶדֶת","descending"],
    ["מֵאָב","from father"],
    ["לְבֵן","to son"],
    ["יוּכְלוּ","they may"],
    ["לִדְרֹשׁ","claim"],
    ["מִשְׁחָתָם","their anointing"],
    ["אִם","if"],
    ["בְּכׇל","at any"],
    ["עֵת","time"],
    ["יוּכְלוּ","they can"],
    ["לְהוֹכִיחַ","prove"],
    ["יַחְשָׂם","their lineage"],
    ["אוֹ","or"],
    ["לְבָרֵר","ascertain"],
    ["אוֹתוֹ","it"],
    ["בְּחָזוֹן","by revelation"],
    ["מֵיְהוָה","from the Lord"],
    ["תַּחַת","under"],
    ["יְדֵי","the hands of"],
    ["הַנְּשִׂיאוּת","the Presidency"],
    ["הַנִּזְכֶּרֶת","named"],
    ["לְמַעְלָה׃","above"]
  ]},
  { num: "כב", words: [["וְעוֹד","and again"], ["לֹא","not"], ["יִשָּׁפֵט","shall be tried"], ["אוֹ","or"], ["יֻרְשַׁע","condemned"], ["פָּקִיד","any bishop"], ["אוֹ","or"], ["כֹּהֵן","priest"], ["גָּדוֹל","high"], ["הַנִּבְדָּל","who is set apart"], ["לָעֲבוֹדָה","for ministry"], ["הַזֹּאת","this"], ["עַל","for"], ["כׇּל","any"], ["פֶּשַׁע","crime"], ["כִּי־אִם","except"], ["לִפְנֵי","before"], ["נְשִׂיאוּת","the First Presidency of"], ["הַכְּנֵסִיָּה׃","the church"]]},
  { num: "כג", words: [
    ["וְאִם","and if"],
    ["יִמָּצֵא","he is found"],
    ["אָשֵׁם","guilty"],
    ["לִפְנֵי","before"],
    ["הַנְּשִׂיאוּת","the Presidency"],
    ["הַזֹּאת","this"],
    ["בְּעֵדוּת","by testimony"],
    ["אֲשֶׁר","that"],
    ["לֹא","cannot"],
    ["יֻפַר","be impeached"],
    ["יֻרְשַׁע׃","he shall be condemned"]
  ]},
  { num: "כד", words: [["וְאִם","and if"], ["יָשׁוּב","he repents"], ["יִסָּלַח","it shall be forgiven"], ["לוֹ","unto him"], ["לְפִי","according to"], ["בְּרִיתוֹת","the covenants"], ["וּמִצְווֹת","and commandments of"], ["הַכְּנֵסִיָּה׃","the church"]]},
  { num: "כה", words: [["וְעוֹד","and again"], ["אִם","inasmuch as"], ["יֵשׁ","there are"], ["יְלָדִים","children"], ["בְּצִיּוֹן","in Zion"], ["אוֹ","or"], ["בְּכׇל","in any of"], ["יְתֵדוֹתֶיהָ","her stakes organized"], ["וְאֵינָם","and they are not"], ["מְלַמְּדִים","teaching"], ["אוֹתָם","them"], ["לְהָבִין","to understand"], ["תּוֹרַת","the doctrine of"], ["תְּשׁוּבָה","repentance"], ["אֱמוּנָה","faith"], ["בַמָּשִׁיחַ","in Christ"], ["בֶּן","the Son of"], ["אֱלֹהִים","the God"], ["חַיִּים","living"], ["וּטְבִילָה","and of baptism"], ["וּמַתַּת","and the gift of"], ["רוּחַ","the Spirit of"], ["הַקֹּדֶשׁ","the Holy"], ["בִּסְמִיכַת","by the laying on of"], ["יָדַיִם","hands"], ["בִּהְיוֹתָם","when they are"], ["בְּנֵי","sons of"], ["שְׁמֹנֶה","eight"], ["שָׁנִים","years"], ["יִהְיֶה","shall be"], ["הַחֵטְא","the sin"], ["עַל","upon"], ["רֹאשׁ","the heads of"], ["הַהוֹרִים׃","the parents"]]},
  { num: "כו", words: [
    ["כִּי","for"],
    ["זֹאת","this"],
    ["תִּהְיֶה","shall be"],
    ["לְתוֹרָה","a law"],
    ["לְיוֹשְׁבֵי","unto the inhabitants of"],
    ["צִיּוֹן","Zion"],
    ["אוֹ","or"],
    ["בְּכׇל","in any of"],
    ["יְתֵדוֹתֶיהָ","her stakes organized"],
    ["הָעֲרוּכוֹת׃","which are organized"]
  ]},
  { num: "כז", words: [
    ["וְיִטָּבְלוּ","and shall be baptized"],
    ["יַלְדֵיהֶם","their children"],
    ["לְסְלִיחַת","for the remission of"],
    ["חֲטָאֵיהֶם","their sins"],
    ["בִּהְיוֹתָם","when they are"],
    ["בְּנֵי","sons of"],
    ["שְׁמֹנֶה","eight"],
    ["שָׁנִים","years"],
    ["וִיקַבְּלוּ","and shall receive"],
    ["סְמִיכַת","the laying on of"],
    ["יָדַיִם׃","hands"]
  ]},
  { num: "כח", words: [
    ["וִילַמְּדוּ","and they shall teach"],
    ["גַם","also"],
    ["יַלְדֵיהֶם","their children"],
    ["לְהִתְפַּלֵּל","to pray"],
    ["וְלָלֶכֶת","and to walk"],
    ["בְּיֹשֶׁר","uprightly"],
    ["לִפְנֵי","before"],
    ["יְהוָה׃","the Lord"]
  ]},
  { num: "כט", words: [
    ["וְיִשְׁמְרוּ","and shall observe"],
    ["יוֹשְׁבֵי","the inhabitants of"],
    ["צִיּוֹן","Zion"],
    ["גַּם","also"],
    ["אֶת־יוֹם","the day of"],
    ["הַשַּׁבָּת","the Sabbath"],
    ["לְקַדְּשׁוֹ׃","to keep it holy"]
  ]},
  { num: "ל", words: [
    ["וְיִזְכְּרוּ","and shall remember"],
    ["יוֹשְׁבֵי","the inhabitants of"],
    ["צִיּוֹן","Zion"],
    ["גַּם","also"],
    ["מְלַאכְתָּם","their labors"],
    ["אִם","inasmuch as"],
    ["מֻנּוּ","they are appointed"],
    ["לַעֲבֹד","to labor"],
    ["בְּכׇל","in all"],
    ["אֱמוּנָה","faithfulness"],
    ["כִּי","for"],
    ["הָעָצֵל","the idler"],
    ["יִזָּכֵר","shall be had in remembrance"],
    ["לִפְנֵי","before"],
    ["יְהוָה׃","the Lord"]
  ]},
  { num: "לא", words: [["עַתָּה","now"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["אֵינֶנִּי","am not"], ["רָצוּי","well pleased"], ["בְּיוֹשְׁבֵי","with the inhabitants of"], ["צִיּוֹן","Zion"], ["כִּי","for"], ["יֵשׁ","there are"], ["עֲצֵלִים","idlers"], ["בֵּינֵיהֶם","among them"], ["וְיַלְדֵיהֶם","and their children"], ["גַּם","also"], ["גְּדֵלִים","are growing up"], ["בְּרֶשַׁע","in wickedness"], ["גַּם","also"], ["אֵינָם","they do not"], ["מְבַקְּשִׁים","seek"], ["בְּכׇל־לֵב","with all the heart"], ["עֹשֶׁר","the riches of"], ["הַנֶּצַח","eternity"], ["כִּי־אִם","but"], ["עֵינֵיהֶם","their eyes are"], ["מְלֵאוֹת","full of"], ["בֶּצַע׃","greediness"]]},
  { num: "לב", words: [
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["לֹא","not"],
    ["יִהְיוּ","ought to be"],
    ["וְנָכוֹן","and must"],
    ["לְהָסִירָם","be done away"],
    ["מִבֵּינֵיהֶם","from among them"],
    ["לָכֵן","wherefore"],
    ["עַבְדִּי","my servant"],
    ["אוֹלִיבֶר","Oliver"],
    ["קַאוּדְרִי","Cowdery"],
    ["יוֹלִיךְ","let him carry"],
    ["הָאֲמָרִים","the sayings"],
    ["הָאֵלֶּה","these"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "לג", words: [
    ["וּמִצְוָה","and a commandment"],
    ["אֲנִי","I"],
    ["נוֹתֵן","give"],
    ["לָהֶם","unto them"],
    ["אֲשֶׁר","that"],
    ["אֵינֶנּוּ","he that not"],
    ["שֹׁמֵר","observes"],
    ["תְּפִלּוֹתָיו","his prayers"],
    ["לִפְנֵי","before"],
    ["יְהוָה","the Lord"],
    ["יִזָּכֵר","let him be had in remembrance"],
    ["לִפְנֵי","before"],
    ["שׁוֹפֵט","the judge of"],
    ["עַמִּי׃","my people"]
  ]},
  { num: "לד", words: [
    ["הָאֲמָרִים","the sayings"],
    ["הָאֵלֶּה","these"],
    ["אֱמֶת","are truth"],
    ["וֶאֱמוּנָה","and faithfulness"],
    ["הֵם","they are"],
    ["לָכֵן","wherefore"],
    ["אַל","do not"],
    ["תַּעַבְרוּ","transgress"],
    ["עֲלֵיהֶם","them"],
    ["וְאַל","neither"],
    ["תִּגְרְעוּ","take"],
    ["מֵהֶם׃","therefrom"]
  ]},
  { num: "לה", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I am"],
    ["אָלֶף","Alpha"],
    ["וְתָו","and Omega"],
    ["וּבָא","and I come"],
    ["אֲנִי","I"],
    ["מְהֵרָה","quickly"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc68_ch1Verses, 'dc68-ch1-verses');



var dc69_ch1Verses = [
  { num: "א", words: [
    ["הַאֲזִינוּ","hearken"],
    ["לִי","unto me"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["אֱלֹהֵיכֶם","your God"],
    ["לְמַעַן","for the sake of"],
    ["עַבְדִּי","my servant"],
    ["אוֹלִיבֶר","Oliver"],
    ["קַאוּדְרִי","Cowdery"],
    ["אֵין","it is not"],
    ["זֶה","this"],
    ["חׇכְמָה","wisdom"],
    ["בִּי","in me"],
    ["כִּי","that"],
    ["יֵאָמֵן","he should be entrusted"],
    ["עָלָיו","upon him"],
    ["הַמִּצְוֹת","with the commandments"],
    ["וְהַכְּסָפִים","and the moneys"],
    ["אֲשֶׁר","which"],
    ["יוֹלִיךְ","he shall carry"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן","Zion"],
    ["כִּי","except"],
    ["אִם","that"],
    ["יֵלֵךְ","there go"],
    ["עִמּוֹ","with him"],
    ["אֶחָד","one"],
    ["אֲשֶׁר","who"],
    ["יִהְיֶה","will be"],
    ["נֶאֱמָן","true"],
    ["וְיָשָׁר׃","and faithful"]
  ]},
  { num: "ב", words: [["לָכֵן","wherefore"], ["אֲנִי","I"], ["יְהוָה","the Lord"], ["חָפֵץ","will"], ["כִּי","that"], ["עַבְדִּי","my servant"], ["ג׳וֹן","John"], ["וִיטְמֵר","Whitmer"], ["יֵלֵךְ","should go"], ["עִם","with"], ["עַבְדִּי","my servant"], ["אוֹלִיבֶר","Oliver"], ["קַאוּדְרִי׃","Cowdery"]]},
  { num: "ג", words: [["וְגַם","and also"], ["כִּי","that"], ["יוֹסִיף","he shall continue"], ["לִכְתֹּב","in writing"], ["וְלַעֲשׂוֹת","and making"], ["דִּבְרֵי","a history of"], ["יְמֵי","the days of"], ["כׇּל","all"], ["הַדְּבָרִים","the things"], ["הַחֲשׁוּבִים","important"], ["אֲשֶׁר","which"], ["יִרְאֶה","he shall observe"], ["וְיֵדַע","and know"], ["עַל","concerning"], ["כְּנֵסִיָּתִי׃","my church"]]},
  { num: "ד", words: [
    ["וְגַם","and also"],
    ["כִּי","that"],
    ["יְקַבֵּל","he receive"],
    ["עֵצָה","counsel"],
    ["וְעֶזְרָה","and assistance"],
    ["מֵעַבְדִּי","from my servant"],
    ["אוֹלִיבֶר","Oliver"],
    ["קַאוּדְרִי","Cowdery"],
    ["וַאֲחֵרִים׃","and others"]
  ]},
  { num: "ה", words: [
    ["וְגַם","and also"],
    ["עֲבָדַי","my servants"],
    ["הַנִּמְצָאִים","who are abroad"],
    ["בָּאָרֶץ","in the earth"],
    ["יִשְׁלְחוּ","should send forth"],
    ["חֶשְׁבּוֹן","the accounts of"],
    ["פְּקֻדָּתָם","their stewardships"],
    ["לְאֶרֶץ","unto the land of"],
    ["צִיּוֹן׃","Zion"]
  ]},
  { num: "ו", words: [
    ["כִּי","for"],
    ["אֶרֶץ","the land of"],
    ["צִיּוֹן","Zion"],
    ["תִּהְיֶה","shall be"],
    ["לְמוֹשָׁב","a seat"],
    ["וְלְמָקוֹם","and a place"],
    ["לְקַבֵּל","to receive"],
    ["וְלַעֲשׂוֹת","and to do"],
    ["כׇּל","all"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה׃","these"]
  ]},
  { num: "ז", words: [["אֲבָל","nevertheless"], ["עַבְדִּי","my servant"], ["ג׳וֹן","John"], ["וִיטְמֵר","Whitmer"], ["יִסַּע","let him travel"], ["פְּעָמִים","times"], ["רַבּוֹת","many"], ["מִמָּקוֹם","from place"], ["לְמָקוֹם","to place"], ["וּמִכְּנֵסִיָּה","and from church"], ["לִכְנֵסִיָּה","to church"], ["לְמַעַן","that"], ["יַשִּׂיג","he may obtain"], ["דַּעַת","knowledge"], ["נָקֵל","easily"], ["יוֹתֵר׃","more"]]},
  { num: "ח", words: [["מַטִּיף","preaching"], ["וּמְבָאֵר","and expounding"], ["כּוֹתֵב","writing"], ["וּמַעְתִּיק","and copying"], ["בּוֹרֵר","selecting"], ["וּמַשִּׂיג","and obtaining"], ["כׇּל","all"], ["הַדְּבָרִים","the things"], ["אֲשֶׁר","which"], ["יִהְיוּ","shall be"], ["לְטוֹבַת","for the good of"], ["הַכְּנֵסִיָּה","the church"], ["וְלַדּוֹרוֹת","and for the generations"], ["הָעוֹלִים","rising"], ["אֲשֶׁר","that"], ["יִגְדְּלוּ","shall grow up"], ["עַל","upon"], ["אֶרֶץ","the land of"], ["צִיּוֹן","Zion"], ["לְרִשְׁתָּהּ","to possess it"], ["מִדּוֹר","from generation"], ["לְדוֹר","to generation"], ["לְעוֹלָם","for ever"], ["וָעֶד","and ever"], ["אָמֵן׃","amen"]]}
];
renderVerseSet(dc69_ch1Verses, 'dc69-ch1-verses');



var dc70_ch1Verses = [
  { num: "א", words: [["הִנֵּה","behold"], ["וְהַאֲזִינוּ","and hearken"], ["יוֹשְׁבֵי","O inhabitants of"], ["צִיּוֹן","Zion"], ["וְכׇל","and all"], ["עַם","the people of"], ["כְּנֵסִיָּתִי","my church"], ["הָרְחוֹקִים","who are afar off"], ["וְשִׁמְעוּ","and hear"], ["אֶת־דְּבַר","the word of"], ["יְהוָה","the Lord"], ["אֲשֶׁר","which"], ["אֲנִי","I"], ["נוֹתֵן","give"], ["לְעַבְדִּי","unto my servant"], ["יוֹסֵף","Joseph"], ["סְמִית","Smith"], ["הַבֵּן","Jun."], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["מַרְטִין","Martin"], ["הֶרִיס","Harris"], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["אוֹלִיבֶר","Oliver"], ["קַאוּדְרִי","Cowdery"], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["ג׳וֹן","John"], ["וִיטְמֵר","Whitmer"], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["סִידְנִי","Sidney"], ["רִיגְדּוֹן","Rigdon"], ["וְגַם","and also"], ["לְעַבְדִּי","unto my servant"], ["וִילְיָם","William"], ["ו.","W."], ["פֶלְפְּס","Phelps"], ["בְּדֶרֶךְ","by way of"], ["מִצְוָה","commandment"], ["אֲלֵיהֶם׃","unto them"]]},
  { num: "ב", words: [
    ["כִּי","for"],
    ["אֲנִי","I"],
    ["נוֹתֵן","give"],
    ["לָהֶם","unto them"],
    ["מִצְוָה","a commandment"],
    ["לָכֵן","wherefore"],
    ["הַאֲזִינוּ","hearken"],
    ["וְשִׁמְעוּ","and hear"],
    ["כִּי","for"],
    ["כֹה","thus"],
    ["אָמַר","says"],
    ["יְהוָה","the Lord"],
    ["אֲלֵיהֶם׃","unto them"]
  ]},
  { num: "ג", words: [
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מִנִּיתִים","have appointed them"],
    ["לִהְיוֹת","to be"],
    ["סֹכְנִים","stewards"],
    ["עַל","over"],
    ["הַחֲזוֹנוֹת","the revelations"],
    ["וְהַמִּצְוֹת","and the commandments"],
    ["אֲשֶׁר","which"],
    ["נָתַתִּי","I have given"],
    ["לָהֶם","unto them"],
    ["וַאֲשֶׁר","and which"],
    ["אֶתֵּן","I shall give"],
    ["לָהֶם","unto them"],
    ["אַחֲרֵי","after"],
    ["כֵן׃","this"]
  ]},
  { num: "ד", words: [
    ["וְחֶשְׁבּוֹן","and an account of"],
    ["הַפְּקֻדָּה","the stewardship"],
    ["הַזֶּה","this"],
    ["אֶדְרֹשׁ","will I require"],
    ["מֵהֶם","of them"],
    ["בְּיוֹם","in the day of"],
    ["הַמִּשְׁפָּט׃","judgment"]
  ]},
  { num: "ה", words: [["לָכֵן","therefore"], ["מִנִּיתִים","I have appointed them"], ["וְזֹאת","and this is"], ["עֲבוֹדָתָם","their ministry"], ["בִּכְנֵסִיַּת","in the church of"], ["אֱלֹהִים","God"], ["לְנַהֵל","to manage"], ["אוֹתָם","them"], ["וְאֶת","and"], ["עִנְיְנֵיהֶם","their concerns"], ["אָכֵן","yea"], ["אֶת־טוֹבוֹתֵיהֶם׃","the benefits thereof"]]},
  { num: "ו", words: [["לָכֵן","wherefore"], ["מִצְוָה","a commandment"], ["אֲנִי","I"], ["נוֹתֵן","give"], ["לָהֶם","unto them"], ["אֲשֶׁר","that"], ["לֹא","not"], ["יִתְּנוּ","they shall give"], ["אֶת־הַדְּבָרִים","the things"], ["הָאֵלֶּה","these"], ["לַכְּנֵסִיָּה","unto the church"], ["וְלֹא","neither"], ["לָעוֹלָם׃","unto the world"]]},
  { num: "ז", words: [
    ["אֲבָל","nevertheless"],
    ["בַּאֲשֶׁר","when"],
    ["יְקַבְּלוּ","that receives him"],
    ["יוֹתֵר","more"],
    ["מֵאֲשֶׁר","than that which"],
    ["נָחוּץ","is needful"],
    ["לְצָרְכֵיהֶם","for their necessities"],
    ["וּלְמַחְסוֹרֵיהֶם","and their wants"],
    ["יִנָּתֵן","it shall be given"],
    ["לְאוֹצָרִי׃","into my storehouse"]
  ]},
  { num: "ח", words: [
    ["וְהַטּוֹבוֹת","and the benefits"],
    ["יֻקְדְּשׁוּ","they shall be consecrated"],
    ["לְיוֹשְׁבֵי","unto the inhabitants of"],
    ["צִיּוֹן","Zion"],
    ["וּלְדוֹרוֹתֵיהֶם","and unto their generations"],
    ["בַּאֲשֶׁר","when"],
    ["יִהְיוּ","they become"],
    ["יוֹרְשִׁים","heirs"],
    ["לְפִי","according to"],
    ["חֻקֵּי","the laws of"],
    ["הַמַּלְכוּת׃","the kingdom"]
  ]},
  { num: "ט", words: [
    ["הִנֵּה","behold"],
    ["זֶה","this is"],
    ["אֲשֶׁר","what"],
    ["יְהוָה","the Lord"],
    ["דּוֹרֵשׁ","requires"],
    ["מִכׇּל","of every"],
    ["אָדָם","man"],
    ["בִּפְקֻדָּתוֹ","in his stewardship"],
    ["כַּאֲשֶׁר","even as"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["מִנִּיתִי","have appointed"],
    ["אוֹ","or"],
    ["אֲמַנֶּה","shall appoint"],
    ["אַחֲרֵי","after"],
    ["כֵן","this"],
    ["לְכׇל","unto any"],
    ["אָדָם׃","man"]
  ]},
  { num: "י", words: [["וְהִנֵּה","and behold"], ["אֵין","there is no"], ["אִישׁ","man"], ["נָקִי","exempt"], ["מִן","from"], ["הַתּוֹרָה","the law"], ["הַזֹּאת","this"], ["אֲשֶׁר","who belongs"], ["לִכְנֵסִיַּת","to the church of"], ["אֱלֹהִים","the God"], ["חַיִּים׃","living"]]},
  { num: "יא", words: [
    ["כֵּן","yea"],
    ["לֹא","neither"],
    ["הַפָּקִיד","the bishop"],
    ["וְלֹא","neither"],
    ["הַסֹּכֵן","the agent"],
    ["הַשּׁוֹמֵר","who keeps"],
    ["עַל","over"],
    ["אוֹצַר","the storehouse of"],
    ["יְהוָה","the Lord"],
    ["וְלֹא","neither"],
    ["הַמְמֻנֶּה","he who is appointed"],
    ["עַל","over"],
    ["פְּקֻדָּה","a stewardship"],
    ["בִּדְבָרִים","in things"],
    ["גַּשְׁמִיִּים׃","temporal"]
  ]},
  { num: "יב", words: [
    ["הַמְמֻנֶּה","he who is appointed"],
    ["לְשָׁרֵת","to administer"],
    ["בִּדְבָרִים","in things"],
    ["רוּחָנִיִּים","spiritual"],
    ["הוּא","the same"],
    ["רָאוּי","is worthy"],
    ["לִשְׂכָרוֹ","of his hire"],
    ["כְּאֵלֶּה","even as those"],
    ["הַמְמֻנִּים","who are appointed"],
    ["עַל","over"],
    ["פְּקֻדָּה","a stewardship"],
    ["לְשָׁרֵת","to administer"],
    ["בִּדְבָרִים","in things"],
    ["גַּשְׁמִיִּים׃","temporal"]
  ]},
  { num: "יג", words: [
    ["כֵּן","yea"],
    ["בְּשֶׁפַע","abundantly"],
    ["יוֹתֵר","more"],
    ["וְהַשֶּׁפַע","and the abundance"],
    ["הַזֶּה","this"],
    ["מֻרְבֶּה","is multiplied"],
    ["לָהֶם","unto them"],
    ["בְּיַד","by the means of"],
    ["הִתְגַּלּוּיוֹת","the manifestations of"],
    ["הָרוּחַ׃","the Spirit"]
  ]},
  { num: "יד", words: [
    ["אֲבָל","nevertheless"],
    ["בִּדְבָרִים","in your temporal things"],
    ["הַגַּשְׁמִיִּים","the temporal"],
    ["תִּהְיוּ","you shall be"],
    ["שָׁוִים","equal"],
    ["וְזֹאת","and this"],
    ["לֹא","not"],
    ["בְּלֵב","with heart"],
    ["רַע","grudging"],
    ["וְאִם","and if"],
    ["לֹא","not"],
    ["כֵן","so"],
    ["שֶׁפַע","the abundance of"],
    ["הִתְגַּלּוּיוֹת","the manifestations of"],
    ["הָרוּחַ","the Spirit"],
    ["יֵעָצֵר׃","shall be withheld"]
  ]},
  { num: "טו", words: [
    ["וְעַתָּה","and now"],
    ["הַמִּצְוָה","the commandment"],
    ["הַזֹּאת","this"],
    ["אֲנִי","I"],
    ["נוֹתֵן","I give"],
    ["לַעֲבָדַי","unto my servants"],
    ["לְטוֹבָתָם","for their benefit"],
    ["בְּעוֹדָם","while they remain"],
    ["לְהִתְגַּלּוּת","for a manifestation of"],
    ["בִּרְכוֹתַי","my blessings"],
    ["עַל","concerning"],
    ["רֹאשֵׁיהֶם","their heads"],
    ["וּלְשָׂכָר","and for a reward"],
    ["עֲמָלָם","of their diligence"],
    ["וּלְבִטְחוֹנָם׃","and for their security"]
  ]},
  { num: "טז", words: [
    ["לְמָזוֹן","for food"],
    ["וּלְמַלְבּוּשׁ","and for raiment"],
    ["לְנַחֲלָה","for an inheritance"],
    ["לְבָתִּים","for houses"],
    ["וּלַאֲדָמוֹת","and for lands"],
    ["בְּכׇל","in whatsoever"],
    ["דֶּרֶךְ","circumstances"],
    ["אֲשֶׁר","that"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אֲשִׂימֵם","shall place them"],
    ["וּלְכׇל","and unto every"],
    ["מָקוֹם","place"],
    ["אֲשֶׁר","that"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["אֶשְׁלָחֵם׃","shall send them"]
  ]},
  { num: "יז", words: [
    ["כִּי","for"],
    ["נֶאֱמָנִים","faithful"],
    ["הָיוּ","they have been"],
    ["עַל","over"],
    ["דְּבָרִים","things"],
    ["רַבִּים","many"],
    ["וְהֵיטִיבוּ","and have done well"],
    ["לַעֲשׂוֹת","to do"],
    ["בַּאֲשֶׁר","inasmuch as"],
    ["לֹא","not"],
    ["חָטְאוּ׃","they have sinned"]
  ]},
  { num: "יח", words: [
    ["הִנֵּה","behold"],
    ["אֲנִי","I"],
    ["יְהוָה","the Lord"],
    ["רַחוּם","am merciful"],
    ["וַאֲבָרְכֵם","and I will bless them"],
    ["וְיָבֹאוּ","and they shall enter"],
    ["בְּשִׂמְחַת","into the joy of"],
    ["הַדְּבָרִים","the things"],
    ["הָאֵלֶּה","these"],
    ["כֵּן","even so"],
    ["יְהִי","let it be"],
    ["אָמֵן׃","amen"]
  ]}
];
renderVerseSet(dc70_ch1Verses, 'dc70-ch1-verses');



})();
