...
(function() {
  ...
  /* Shared with bom.html through xref_common.js. The engine adds the Book of
   Mormon's own fifteen: from the other five volumes a BOM reference is a
   cross-volume link, so it must resolve here. bom.html deliberately does not
   add them — there the same reference is an in-page jump through parseBomRef. */
var _abbrToFullBook = Object.assign({}, (window.SWXref || {}).BOOK_ABBREV,
                                        (window.SWXref || {}).BOM_BOOK_ABBREV);
...
function parseScriptureRef(refText) {
    return window.SWXref.parseScriptureRefWith(_abbrToFullBook, refText);
  }

  // Accept full book names too (e.g. "1 John 3:4")
  /* Folded into SWXref.resolveRefKey, which accepts the abbreviation, the same
   name without its period, and the full book name in one pass. */
  ...
  function classifyRefText(refText) {
    ...
    // Scripture: abbreviation, the same without its period, or the full name.
    // Shared with bom.html — see xref_common.js.
    var k = window.SWXref.resolveRefKey(_abbrToFullBook, norm);
